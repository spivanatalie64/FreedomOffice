...
$(gb_CustomTarget_workdir)/odk/html/%.html : $(SRCDIR)/odk/%.html
	...
	sed -e 's|%PRODUCTNAME%|$(PRODUCTNAME)|g' \
	    ...
	    -e 's|%DOXYGEN_PREFIX0%|$(if $(DOXYGEN),.,https://api.freedomoffice.org)|g' \
	    -e 's|%DOXYGEN_PREFIX1%|$(if $(DOXYGEN),..,https://api.freedomoffice.org)|g' \
	    -e 's|%DOXYGEN_PREFIX2%|$(if $(DOXYGEN),../..,https://api.freedomoffice.org)|g' \
	    -e 's|%JAVADOC_PREFIX0%|$(if $(ENABLE_JAVA),.,https://api.freedomoffice.org)|g' \
	    -e 's|%JAVADOC_PREFIX1%|$(if $(ENABLE_JAVA),..,https://api.freedomoffice.org)|g' \
	    < $< > $@
