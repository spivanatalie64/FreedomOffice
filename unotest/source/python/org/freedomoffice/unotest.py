...
class OfficeConnection(object):
    ...
    def setUp(self):
        ...
        self.FREEDOMOFFICE = self.bootstrap(prog, userdir, channel)
        return self.connect(channel)

    def bootstrap(self, FREEDOMOFFICE, userdir, channel):
        argv = [ FREEDOMOFFICE, "--accept=" + channel + ";urp",
        ...
        if self.verbose:
            print ("starting FreedomOffice with channel: ", channel)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Help utilities for testing FreedomOffice")
    ...
    if args.verbose:
        verbose = True
    from uitest.freedomoffice.connection import PersistentConnection
    con = PersistentConnection({"verbose" : args.verbose})
    print("starting FREEDOMOFFICE ... ", end="")
    con.setUp()
    print("done")
    con.get
