...
print_stamp 1 start
echo git: `git -C $SRC/freedomoffice log -1 --pretty=reference`
...
export CXX="$CXX -stdlib=libc++ -D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_FAST -fsanitize-blacklist=$SRC/freedomoffice/bin/sanitize-excludelist.txt"
export CXX_FOR_BUILD="$CXX"
export CC="$CC -fsanitize-blacklist=$SRC/freedomoffice/bin/sanitize-excludelist.txt"
...
$SRC/freedomoffice/autogen.sh --with-distro=FreedomOfficeOssFuzz --with-external-tar=$SRC/external-tar
...
#options files
cp $SRC/freedomoffice/vcl/workben/*.options $OUT
