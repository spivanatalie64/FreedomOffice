...
class OfficeConnection:
    ...
    def setUp(self):
        ...
        if method == "path":
                ...
                self.FREEDOMOFFICE = self.bootstrap(rest, userdir, self.socket)
        elif method == "connect":
                self.socket = rest
        else:
...
class WatchDog(threading.Thread):
    ...
    def run(self):
        try:
            if self.connection.FREEDOMOFFICE: # not possible for "connect"
                self.connection.FREEDOMOFFICE.wait(timeout=120) # 2 minutes?
        except subprocess.TimeoutExpired:
            log("WatchDog: TIMEOUT -> killing FREEDOMOFFICE")
            self.connection.FREEDOMOFFICE.terminate() # actually killing oosplash...
            self.connection.xContext = None
            log("WatchDog: killed FREEDOMOFFICE")
...
def parseArgs(argv):
    (optlist,args) = getopt.getopt(argv[1:], "hr",
            ["help", "FREEDOMOFFICE=", "userdir=", "reference", "valgrind"])
#    print optlist
