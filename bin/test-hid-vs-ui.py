...
def init_hids():
    ...
    if local_repo:
        ...
        if not os.path.exists(os.path.join(repo_dir,'config')):
            subprocess.call(['git','clone','--bare','git://gerrit.freedomoffice.org/help',repo_dir])
...
def init_core_files():
    ...
    if not os.path.exists(os.path.join(core_repo_dir,'.git')):
        subprocess.call(['git','clone','git://gerrit.freedomoffice.org/core',core_repo_dir])
    elif not args['git_static']:
        subprocess.call(['git','fetch','origin'])
...
if __name__ == "__main__":
    ...
    if args['send_to']:
        msg_from = os.path.basename(sys.argv[0]) + '@freedomoffice.org'
...
Your friendly FreedomOffice Help-ids Checker

Note: The bot generating this message can be found and improved here:
       https://gerrit.freedomoffice.org/gitweb?p=dev-tools.git;a=blob;f=scripts/test-hid-vs-ui.py'''
        ...
        msg['Subject'] = 'FreedomOffice Gerrit News for python on %s' % (now.date().isoformat())
        msg['Reply-To'] = msg_to[0]
        msg['X-Mailer'] = 'FreedomOfficeGerritDigestMailer 1.1'
