...
public:
    ...
    {
        return !comphelper::FreedomOfficeKit::isActive() || m_nDisabled != 0;
    }
...
int SfxLokHelper::createView(SfxViewFrame& rViewFrame, ViewShellDocId docId)
{
    ...
    comphelper::FreedomOfficeKit::setDocId(docId);
...
void SfxLokHelper::setView(int nId)
{
    ...
    if (bIsCurrShell && comphelper::FreedomOfficeKit::getLanguageTag().getBcp47() == pViewShell->GetLOKLanguageTag().getBcp47())
        return;
    ...
    if (bIsCurrShell)
    {
        ...
        SAL_WARN("lok", "LANGUAGE mismatch at setView! ... old (wrong) lang:"
                        << comphelper::FreedomOfficeKit::getLanguageTag().getBcp47()
                        << " new lang:" << pViewShell->GetLOKLanguageTag().getBcp47());
    }

    // update the current LOK language and locale for the dialog tunneling
    comphelper::FreedomOfficeKit::setLanguageTag(pViewShell->GetLOKLanguageTag());
    comphelper::FreedomOfficeKit::setLocale(pViewShell->GetLOKLocale());
...
void SfxLokHelper::setViewLanguage(int nId, const OUString& rBcp47LanguageTag)
{
    if (SfxViewShell* pViewShell = getViewOfId(nId))
    {
        ...
        bool bIsCurrShell = (pViewShell == SfxViewShell::Current());
        if (bIsCurrShell)
            comphelper::FreedomOfficeKit::setLanguageTag(LanguageTag(rBcp47LanguageTag));
    }
}
...
void SfxLokHelper::setViewLocale(int nId, const OUString& rBcp47LanguageTag)
{
    if (SfxViewShell* pViewShell = getViewOfId(nId))
    {
        ...
        if (pViewShell->GetViewShellId() == ViewShellId(nId))
        {
            ...
            if (bIsCurrShell)
            {
                comphelper::FreedomOfficeKit::setLocale(LanguageTag(rBcp47LanguageTag));
            }
            return;
        }
    }
...
{
    // -1 means all parts
    const int nPart = comphelper::FreedomOfficeKit::isPartInInvalidation() ? pThisView->getPart() : INT_MIN;
...
void SfxLokHelper::notifyInvalidation(SfxViewShell const* pThisView, const int nInPart, tools::Rectangle const* pRect)
{
    ...
    // -1 means all parts
    const int nPart = comphelper::FreedomOfficeKit::isPartInInvalidation() ? nInPart : INT_MIN;
...
{
    if (comphelper::FreedomOfficeKit::isViewIdForVisCursorInvalidation())
    {
...
void SfxLokHelper::notifyLog(const std::ostringstream& stream)
{
    ...
    if (!pViewShell)
       return;
    if (pViewShell->getFreedomOfficeKitViewCallback())
    {
...
{
    comphelper::FreedomOfficeKit::setViewSetter([](int nView) {
        SfxLokHelper::setView(nView);
    });
    comphelper::FreedomOfficeKit::setViewGetter([]() -> int {
...
void SfxLokHelper::dumpState(rtl::OStringBuffer &rState)
{
    ...
    {
        if (pCurrentViewShell == nullptr || pViewShell->GetDocId() == pCurrentViewShell-> GetDocId())
            pViewShell->dumpFreedomOfficeKitViewState(rState);
    ...
    , m_pOldShell(nullptr)
{
    ...
    if (!comphelper::FreedomOfficeKit::isActive() || !pNewShell || pNewShell == m_pOldShell)
    {
        return;
    }
    ...
    // language/locale.
    comphelper::FreedomOfficeKit::setLanguageTag(pNewShell->GetLOKLanguageTag());
    comphelper::FreedomOfficeKit::setLocale(pNewShell->GetLOKLocale());
    m_bSetLanguage = true;
}
...
SfxLokLanguageGuard::~SfxLokLanguageGuard()
{
    ...
    comphelper::FreedomOfficeKit::setLanguageTag(m_pOldShell->GetLOKLanguageTag());
    comphelper::FreedomOfficeKit::setLocale(m_pOldShell->GetLOKLocale());
}
