...
emscripten_install_files := \
    favicon.ico \
    FREEDOMOFFICE.data \
    FREEDOMOFFICE.data.js.metadata \
    FREEDOMOFFICE.js \
    FREEDOMOFFICE.wasm \
    $(if $(EMSCRIPTEN_WORKERJS),FREEDOMOFFICE.worker.js) \
    $(if $(ENABLE_SYMBOLS_FOR), \
        FREEDOMOFFICE.wasm.debug.wasm \
        FREEDOMOFFICE.wasm.debug.wasm.dwp) \
    $(if $(DISABLE_GUI),, \
        qt_soffice.html \
        qtloader.js \
        qtlogo.svg) \
...
$(call gb_CustomTarget_get_target,instsetoo_native/emscripten-install): \
	...
	mkdir -p $(WORKDIR)/installation/FreedomOffice/emscripten
	for i in $(emscripten_install_files); do \
        cp $(INSTDIR)/program/$$i $(WORKDIR)/installation/FreedomOffice/emscripten/ || exit 1; \
    done
