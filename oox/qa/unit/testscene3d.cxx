...
{
    // The legacy preset camera types correspond to the geometry available in the UI of FreedomOffice.
...
CPPUNIT_TEST_FIXTURE(TestScene3d, test_obliqueTopRight)
{
    ...
    // That conversion results in angles not available in the UI of FreedomOffice, but LO can
...
CPPUNIT_TEST_FIXTURE(TestScene3d, test_perspectiveContrastingLeft)
{
    ...
    // Such shape cannot be produced in the UI of FreedomOffice, but it can be rendered.
...
CPPUNIT_TEST_FIXTURE(TestScene3d, test_legacyPerspectiveTopRight)
{
    ...
    // camera corresponds directly to the extrusions available in the UI of FreedomOffice. The
...
CPPUNIT_TEST_FIXTURE(TestScene3d, test_lightRig_default)
{
    ...
    // second light is against view direction, the colors are same in FreedomOffice and MS Office.
    // The test assumes rendering with ShadeMode_FLAT.
...
CPPUNIT_TEST_FIXTURE(TestScene3d, test_lightRig_dir_rotation)
{
    ...
    // direction of the second light is against the view direction, colors are same in FreedomOffice
