...
public:
    ...
    virtual void Invoke() override
    {
        ...
        {
            TOOLS_INFO_EXCEPTION("svl", "trying to install a FreedomOffice langpack");
        }
            ...
            if (!install.isEmpty() && install != "en-US")
            {
                ...
                if (sVendor == "Red Hat, Inc." || sVendor == "The Fedora Project")
                {
                    ...
                    if (MsLangId::isSimplifiedChinese(eType))
                        aPackages.emplace_back("freedomoffice-langpack-zh-Hans");
                    else if (MsLangId::isTraditionalChinese(eType))
                        aPackages.emplace_back("freedomoffice-langpack-zh-Hant");
                    else if (install == "pt")
                        aPackages.emplace_back("freedomoffice-langpack-pt-PT");
                    else
                        aPackages.emplace_back("freedomoffice-langpack-" + install);
                }
                else if (sVendor == "Freedom Foundation/Debian" || sVendor == "Freedom Foundation, Debian and Ubuntu")
                {
                    // l10n is the typical Debian/Ubuntu naming convention
                    aPackages.emplace_back("freedomoffice-l10n-" + install);
                }
            }
            if (!aPackages.empty())
            {
