# FreedomOffice for Android

## Bootstrap

Contains common code for all projects on Android to bootstrap FreedomOffice. In
addition it is a home to `FreedomOfficeKit` (LOK - see `freedomofficekit/README.md`) JNI
classes.

## Stuff in Source Directory

FreedomOffice Android application - the code is based on Fennec (Firefox for Android).
It uses OpenGL ES 2 for rendering of the document tiles which are gathered from
FreedomOffice using LOK. The application contains the FreedomOffice core in one shared
...
3. A thread (`LOKitThread`), that performs `FreedomOfficeKit` calls, which may take more time
   to complete. In addition it also receives events from the FREEDOMOFFICE thread (see below)
...
4. A native thread created by FreedomOfficeKit (we call it the FREEDOMOFFICE thread), where
   FreedomOffice itself runs. It receives calls from `LOKitThread`, and may emit callback
   events as necessary.

## LOKitThread

`LOKitThread` (`org.freedomoffice.LOKitThread`) communicates with LO via JNI (this can
be done only for one thread) and processes events (defined in `org.freedomoffice.LOEvent`)
triggered from UI.

## Application Overview

FreedomOfficeMainActivity (`org.freedomoffice.FreedomOfficeMainActivity`) is the entry point
...
Tile can change in FreedomOffice when user changes the content (adds, removes text or changes
the properties). In this case, an invalidation rectangle is signaled from FreedomOffice, which
...
For editing there are 2 coarse tasks that the FreedomOffice app must do:

1. send input events to FreedomOffice core (keyboard, touch and mouse)
2. listen to messages (provided via callback) from FreedomOffice core and react accordingly
...
processed and sent to FreedomOffice core. The touch events originate in `JavaPanZoomController`,
the keyboard events in `LOKitInputConnectionHandler` (`org.freedomoffice.LOKitInputConnectionHandler`),
however there are other parts too - depending on the need.

`InvalidationHandler` (`org.freedomoffice.InvalidationHandler`) is the class that is responsible
to process messages from FreedomOffice core and to track the state.
...
`DocumentOverlay` (`org.freedomoffice.overlay.DocumentOverlay`) and `DocumentOverlayView`
(`org.freedomoffice.overlay.DocumentOverlayView`) are the classes that provide the overlay over
...
- use Run|Edit Configurations to create a new configuration of type "Android Native"
	...
	"command script import `/path/to/solenv/lldb/freedomoffice/LO.py`"
...
FreedomOffice in order to reduce the size of the apk. When developing,
...
### Common Errors / Gotchas
    ...
    lo_dlneeds: Could not read ELF header of /data/data/org.freedomoffice...libfoo.so
