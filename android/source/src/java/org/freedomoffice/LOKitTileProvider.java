...
import org.freedomoffice.kit.DirectBufferAllocator;
import org.freedomoffice.kit.Document;
import org.freedomoffice.kit.FreedomOfficeKit;
import org.freedomoffice.kit.Office;
...
class LOKitTileProvider implements TileProvider {
    ...
    private final FreedomOfficeMainActivity mContext;
     ...
     */
    LOKitTileProvider(FreedomOfficeMainActivity context, InvalidationHandler messageCallback, String input) {
        mContext = context;
        mMessageCallback = messageCallback;

        FreedomOfficeKit.putenv("SAL_LOG=+WARN+INFO");
        FreedomOfficeKit.putenv("SAL_LOK_OPTIONS=compact_fonts");
        FreedomOfficeKit.init(mContext);

        mOffice = new Office(FreedomOfficeKit.getFreedomOfficeKitHandle());
        ...
        if (mDocument == null && !mContext.isPasswordProtected()) {
            ...
            ByteBuffer handle = FreedomOfficeKit.getFreedomOfficeKitHandle();
            Log.i(LOGTAG, "====> getFreedomOfficeKitHandle() = " + handle);
    ...
    /**
     * @see org.freedomoffice.TileProvider#setGraphicSelectionStart(android.graphics.PointF)
     */
    @Override
    ...
    /**
     * @see org.freedomoffice.TileProvider#setGraphicSelectionEnd(android.graphics.PointF)
     */
    @Override
    ...
    private void setGraphicSelection(int type, PointF documentCoordinate) {
        int x = (int) pixelToTwip(documentCoordinate.x, mDPI);
        int y = (int) pixelToTwip(documentCoordinate.y, mDPI);
        FreedomOfficeMainActivity.setDocumentChanged(true);
