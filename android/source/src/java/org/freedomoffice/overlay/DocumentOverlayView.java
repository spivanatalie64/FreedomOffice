 ...
 */
package org.freedomoffice.overlay;
...
import org.freedomoffice.canvas.AdjustLengthLine;
import org.freedomoffice.canvas.CalcSelectionBox;
import org.freedomoffice.canvas.Cursor;
import org.freedomoffice.canvas.GraphicSelection;
import org.freedomoffice.canvas.PageNumberRect;
import org.freedomoffice.canvas.SelectionHandle;
import org.freedomoffice.canvas.SelectionHandleEnd;
import org.freedomoffice.canvas.SelectionHandleMiddle;
import org.freedomoffice.canvas.SelectionHandleStart;
...
public class DocumentOverlayView extends View implements View.OnTouchListener {
    ...
    public void initialize(LayerView layerView) {
        if (!mInitialized) {
            ...
            mGraphicSelection = new GraphicSelection((FreedomOfficeMainActivity) getContext());
            mGraphicSelection.setVisible(false);

            postDelayed(cursorAnimation, CURSOR_BLINK_TIME);

            mHandleMiddle = new SelectionHandleMiddle((FreedomOfficeMainActivity) getContext());
            mHandleStart = new SelectionHandleStart((FreedomOfficeMainActivity) getContext());
            mHandleEnd = new SelectionHandleEnd((FreedomOfficeMainActivity) getContext());
    ...
    public void setCalcHeadersController(CalcHeadersController calcHeadersController) {
        ...
        mCalcSelectionBox = new CalcSelectionBox((FreedomOfficeMainActivity) getContext());
    }
    ...
    public void showAdjustLengthLine(boolean isRow, final CalcHeadersView view) {
        mAdjustLengthLine = new AdjustLengthLine((FreedomOfficeMainActivity) getContext(), view, isRow, getWidth(), getHeight());
        ImmutableViewportMetrics metrics = mLayerView.getViewportMetrics();
