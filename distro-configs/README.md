...
FreedomOffice for various platforms. They are *not* supposed to
...
See <https://wiki.freedomfoundation.org/Development/ReleaseBuilds> for how
...
files specify it, except for `FreedomOfficeLinux.conf`, where the TDF build
instructions pass an explicit `--with-package-format="rpm deb"` in addition to
`--with-distro=FreedomOfficeLinux`.)
...
on the `autogen.sh` command line thus:
    ...
    ./autogen.sh --with-distro=FreedomOfficeFoo
