...
$(call gb_ExternalProject_get_state_target,python3,build) :
	...
	$(call gb_ExternalProject_run,build,\
		...
		--with-system-expat \
		$(if $(filter MACOSX,$(OS)), \
			--enable-framework=/@__________________________________________________OOO --with-framework-name=FreedomOfficePython, \
			--enable-shared \
		) \
...
python3_fw_prefix:=$(gb_UnpackedTarball_workdir)/python3/python-inst/@__________________________________________________OOO/FreedomOfficePython.framework/Versions/$(PYTHON_VERSION_MAJOR).$(PYTHON_VERSION_MINOR)
...
cd \"$$origpath\"\n\
\"$$bindir/../Resources/Python.app/Contents/MacOS/FreedomOfficePython\" - $$@ <<EOF"} \
		FNR==1{next} \
		      {print} \
...
$(call gb_ExternalProject_get_state_target,python3,fixinstallnames) : $(call gb_ExternalProject_get_state_target,python3,build) \
	...
	$(INSTALL_NAME_TOOL) -change \
		$(python3_fw_prefix)/FreedomOfficePython \
		@executable_path/../../../../FreedomOfficePython \
		$(python3_fw_prefix)/Resources/Python.app/Contents/MacOS/FreedomOfficePython
	for file in $(shell $(FIND) $(python3_fw_prefix)/lib/python$(PYTHON_VERSION_MAJOR).$(PYTHON_VERSION_MINOR)/lib-dynload -name "*.so") ; do \
	$(INSTALL_NAME_TOOL) -change \
		$(python3_fw_prefix)/FreedomOfficePython \
		@loader_path/../../../FreedomOfficePython $$file ; done
	touch $@
...
$(call gb_ExternalProject_get_state_target,python3,executables) : $(call gb_ExternalProject_get_state_target,python3,build)
	...
	$(INSTALL_NAME_TOOL) -change \
		$(python3_fw_prefix)/FreedomOfficePython \
		@executable_path/../FreedomOfficePython python$(PYTHON_VERSION_MAJOR).$(PYTHON_VERSION_MINOR)
	touch $@
