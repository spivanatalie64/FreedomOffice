...
#include <FreedomOfficeKit/FreedomOfficeKitInit.h>
#include <FreedomOfficeKit/FreedomOfficeKitEnums.h>
#include <FreedomOfficeKit/FreedomOfficeKitGtk.h>
...
#define CURSOR_HANDLE_DIR "/../share/freedomofficekit/"
...
struct LOKDocViewPrivateImpl
{
    ...
    bool m_bCanZoomOut;
    bool m_bUnipoll;
    FreedomOfficeKit* m_pOffice;
    FreedomOfficeKitDocument* m_pDocument;
    ...
    FreedomOfficeKitDocumentType m_eDocumentType;
...
void setDocumentView(FreedomOfficeKitDocument* pDoc, int viewId)
{
    assert(pDoc);
    std::stringstream ss;
...
/// Helper struct used to pass the data from FREEDOMOFFICE thread -> main thread.
struct CallbackData
{
    int m_nType;
...
callback (gpointer pData)
{
    ...
    switch (static_cast<FreedomOfficeKitCallbackType>(pCallback->m_nType))
    {
...
openDocumentInThread (gpointer data)
{
    ...
    {
        priv->m_eDocumentType = static_cast<FreedomOfficeKitDocumentType>(priv->m_pDocument->pClass->getDocumentType(priv->m_pDocument));
...
static void lok_doc_view_set_property (GObject* object, guint propId, const GValue *value, GParamSpec *pspec)
{
    ...
    case PROP_LO_POINTER:
        priv->m_pOffice = static_cast<FreedomOfficeKit*>(g_value_get_pointer(value));
        break;
    ...
    case PROP_DOC_POINTER:
        priv->m_pDocument = static_cast<FreedomOfficeKitDocument*>(g_value_get_pointer(value));
        priv->m_eDocumentType = static_cast<FreedomOfficeKitDocumentType>(priv->m_pDocument->pClass->getDocumentType(priv->m_pDocument));
        break;
    case PROP_EDITABLE:
...
static gboolean lok_doc_view_initable_init (GInitable *initable, GCancellable* /*cancellable*/, GError **error)
{
    ...
    if (priv->m_pOffice == nullptr)
    {
        g_set_error (error,
                     ...
                     "Failed to get FreedomOfficeKit context. Make sure path (%s) is correct",
...
static void lok_doc_view_class_init (LOKDocViewClass* pClass)
{
     ...
     * LOKDocView:lopath:
     *
     * The absolute path of the FreedomOffice install.
     */
    properties[PROP_LO_PATH] =
        g_param_spec_string("lopath",
                            "LO Path",
                            "FreedomOffice Install Path",
                            nullptr,
     ...
     *
     * A FreedomOfficeKit* in case lok_init() is already called
     * previously.
     */
    properties[PROP_LO_POINTER] =
        g_param_spec_pointer("lopointer",
                             "LO Pointer",
                             "A FreedomOfficeKit* from lok_init()",
                             static_cast<GParamFlags>(G_PARAM_READWRITE |
                                                      G_PARAM_CONSTRUCT_ONLY |
                                                      G_PARAM_STATIC_STRINGS));
     ...
     *
     * The absolute path of the FreedomOffice user profile.
     */
    properties[PROP_USER_PROFILE_URL] =
        g_param_spec_string("userprofileurl",
                            "User profile path",
                            "FreedomOffice user profile path",
                            nullptr,
                            static_cast<GParamFlags>(G_PARAM_READWRITE |
                                                     G_PARAM_CONSTRUCT_ONLY |
                                                     G_PARAM_STATIC_STRINGS));
     ...
     *
     * A FreedomOfficeKitDocument* in case documentLoad() is already called
     * previously.
     */
    properties[PROP_DOC_POINTER] =
        g_param_spec_pointer("docpointer",
                             "Document Pointer",
                             "A FreedomOfficeKitDocument* from documentLoad()",
                             static_cast<GParamFlags>(G_PARAM_READWRITE |
                                                      G_PARAM_STATIC_STRINGS));
                                                             ...
                                                             const gchar* pRenderingArguments)
{
    ...
    FreedomOfficeKitDocument* pDocument = lok_doc_view_get_document(LOK_DOC_VIEW(pNewDocView));
...
SAL_DLLPUBLIC_EXPORT FreedomOfficeKitDocument*
                                 ...
                                 const gchar* pCommand)
{
    ...
    FreedomOfficeKitDocument* pDocument = lok_doc_view_get_document(pDocView);
    if (!pDocument)
        return nullptr;
...
{
    LOKDocViewPrivate& priv = getPrivate(pDocView);
    FreedomOfficeKitDocument* pDocument = priv->m_pDocument;
    bool ret = false;
