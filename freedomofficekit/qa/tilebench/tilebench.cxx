...
static int help( const char *error = nullptr )
{
    ...
    fprintf( stderr, "Usage: tilebench <absolute-path-to-freedomoffice-install> [path to document] [--preinit] [--save <path>] <options>\n");
            ...
            else
            {
                switch (mode)
                {
                    ...
                    case LOK_TILEMODE_BGRA:
                        ...
                        break;
                    default:
                        assert(false && "unhandled FreedomOfficeKitTileMode");
                        break;
                }
            }
...
int main( int argc, char* argv[] )
{
    ...
    if ( argv[1][0] != '/' )
    {
        fprintf(stderr, "Absolute path required to freedomoffice install\n");
        return 1;
    }
