...
#include <FreedomOfficeKit/FreedomOfficeKit.hxx>
...
{
    const string sDocPath = m_sSrcRoot + "/freedomofficekit/qa/data/IDONOTEXIST.odt";
    ...
    // "Unsupported URL <file:///SRC_ROOT/freedomofficekit/qa/data/IDONOTEXIST.odt>: "type detection failed""
}

// Our dumped .png files end up in
// workdir/CppunitTest/freedomofficekit_tiledrendering.test.core
...
{
    const string sDocPath = m_sSrcRoot + "/freedomofficekit/qa/data/" + pName;
    const string sLockFile = m_sSrcRoot +"/freedomofficekit/qa/data/.~lock." + pName + "#";
...
void TiledRenderingTest::testDocumentTypes( Office* pOffice )
{
    ...
    CPPUNIT_ASSERT(pDocument);
    CPPUNIT_ASSERT_EQUAL(LOK_DOCTYPE_TEXT, static_cast<FreedomOfficeKitDocumentType>(pDocument->getDocumentType()));
    // This crashed.
    pDocument->postUnoCommand(".uno:Bold");
    processEventsToIdle();

    const string sPresentationDocPath = m_sSrcRoot + "/freedomofficekit/qa/data/blank_presentation.odp";
    const string sPresentationLockFile = m_sSrcRoot +"/freedomofficekit/qa/data/.~lock.blank_presentation.odp#";

    // FIXME: same comment as below wrt lockfile removal.
    remove( sPresentationLockFile.c_str() );

    CPPUNIT_ASSERT_EQUAL(LOK_DOCTYPE_PRESENTATION, static_cast<FreedomOfficeKitDocumentType>(getDocumentType(pOffice, sPresentationDocPath)));
