...
FreedomOfficeKit can be used for accessing FreedomOffice functionality
...
LOK functionality can be accessed by including `FreedomOfficeKit.h[xx]` in your
program.

LOK initialisation (`lok_init`) requires the inclusion of `FreedomOfficeKitInit.h` in
your program. If you use the C++ `FreedomOfficeKit.hxx` header, it already includes
`FreedomOfficeKitInit.h` for you.

(`FreedomOfficeKit.hxx` is a simple and fully inlined C++ wrapper for the same
functionality as in `FreedomOfficeKit.h`.)
...
Scanlines are ordered top-down (whereas FreedomOffice will internally default
to bottom-up).
...
client and FreedomOffice is a two-way channel. The client can initiate an action
by calling the above mentioned methods. The most important methods for the
client -> FreedomOffice communication are:
...
1" = 1440 twips). See `lok::Document` in `FreedomOfficeKit.hxx` for a full list of
methods and their documentation.

The other way around (FreedomOffice -> LOK client) is implemented using a
...
`FreedomOfficeKitCallbackType` enumeration in `FreedomOfficeKitEnums.h`, the callback
function signature itself is provided by the FreedomOfficeKitCallback typedef in
`FreedomOfficeKitTypes.h`. The most important callback types:
...
Introducing explicit new API under `include/FreedomOfficeKit/` adds type safety but listing each &
