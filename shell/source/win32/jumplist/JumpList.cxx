...
void SAL_CALL JumpListImpl::beginList(const OUString& sApplication)
{
    ...
    OUString sApplicationID("TheDocumentFoundation.FreedomOffice." + sApplication);
                                           ...
                                           const Sequence<JumpListItem>& aJumpListItems)
{
    ...
    try
    {
        OUString freedomofficeURL;
        OUString freedomofficePath;
        oslProcessError err = osl_getExecutableFile(&freedomofficeURL.pData);
        FileBase::getSystemPathFromFileURL(freedomofficeURL, freedomofficePath);
        if (err != osl_Process_E_None)
        {
            SAL_WARN("shell.jumplist", "osl_getExecutableFile failed");
            return;
        }
        // We need to run FREEDOMOFFICE.EXE, not FREEDOMOFFICE.BIN
        freedomofficePath = freedomofficePath.replaceFirst("FREEDOMOFFICE.BIN", "FREEDOMOFFICE.EXE");
        ...
        for (auto const& item : aJumpListItems)
        {
            ...
            try
            {
                ...
                ThrowIfFailed(pShellLinkItem->SetPath(o3tl::toW(freedomofficePath.getStr())),
                              Concat2View("Setting path '" + freedomofficePath.toUtf8() + "' failed."));
