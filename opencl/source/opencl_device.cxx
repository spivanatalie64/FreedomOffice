...
struct FreedomOfficeDeviceEvaluationIO
{
...
/* Populate input */
void populateInput(std::unique_ptr<FreedomOfficeDeviceEvaluationIO> const & testData)
{
...
/* Evaluate devices */
ds_status evaluateScoreForDevice(ds_device& rDevice, std::unique_ptr<FreedomOfficeDeviceEvaluationIO> const & testData)
{
...
ds_status profileDevices(std::unique_ptr<ds_profile> const & pProfile, std::unique_ptr<FreedomOfficeDeviceEvaluationIO> const & pTestData)
{
    ...
    std::u16string_view sProfilePath, bool bForceSelection)
{
    ...
    if (!bIsDeviceSelected || bForceSelection)
    {
        ...
        ds_status status;
        status = initDSProfile(aProfile, "FreedomOffice v1"_ostr);
        ...
        if (DS_SUCCESS != status)
        {
            ...
            std::unique_ptr<FreedomOfficeDeviceEvaluationIO> testData(new FreedomOfficeDeviceEvaluationIO);
