...
# UITests are much more likely to generate core files for the FREEDOMOFFICE than for
...
# the FREEDOMOFFICE executable rather than to gb_UITest_EXECUTABLE:
...
gb_UITest_SOFFICEARG:=path:$(INSTROOT)/$(LIBO_BIN_FOLDER)/FREEDOMOFFICE
endif
...
ifneq ($(gb_SUPPRESS_TESTS),)
	...
	$(call gb_Helper_abbreviate_dirs,\
		...
		$(DEFS) \
		$(if $(filter WNT,$(OS)),SAL_LOG_FILE="$(dir $(call gb_UITest_get_target,$*))FREEDOMOFFICE.out.log") \
		...
		$(gb_TEST_ENV_VARS) \
		$(gb_UITest_COMMAND) \
		--FREEDOMOFFICE="$(gb_UITest_SOFFICEARG)" \
		...
		$(if $(gb_UITest__interactive),, \
		    ...
		    || ($(if $(value gb_CppunitTest_postprocess), \
                ...
                $(if $(filter WNT,$(OS)), \
                    printf '%s: <<<\n' $(dir $(call gb_UITest_get_target,$*))FREEDOMOFFICE.out.log; \
                    cat $(dir $(call gb_UITest_get_target,$*))FREEDOMOFFICE.out.log; \
                    printf ' >>>\n\n';) \
