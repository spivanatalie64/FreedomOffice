= Uploading FreedomOffice API to Maven Central

This file documents the prerequisites and workflow to upload FreedomOffice
API to Maven Central or local Maven repository.

To install FreedomOffice API to local Maven repository or deploy the API
...
must be granted to FreedomOffice project on OSSRH.
...
== Preparations to publish FreedomOffice API to Maven Central
...
* Request permissions to upload artifacts to the `org.freedomoffice`
...
Before publishing new artifacts to Maven Central, `FREEDOMOFFICE_VERSION`
...
Build FreedomOffice as usually, so that API JARs are created.


== Publish the FreedomOffice artifacts to local Maven repository

Execute this command to install FreedomOffice API to your local Maven
...
Once executed, the local Maven repository contains the FreedomOffice API
artifacts:

----
  $ ls -1 ~/.m2/repository/org/freedomoffice/unoil/5.1.0/
  _maven.repositories
  unoil-5.1.0.jar
...
== Publish the FreedomOffice artifacts to Maven Central
...
* To where the artifacts are uploaded depends on the `FREEDOMOFFICE_VERSION`
in the `VERSION` file:

** SNAPSHOT versions are directly uploaded into the Sonatype snapshots
repository and no further action is needed:

https://oss.sonatype.org/content/repositories/snapshots/org/freedomoffice/
...
'Staging Repositories' and find the `orgfreedomoffice-XXXX` staging
repository.
...
section, e.g. https://oss.sonatype.org/content/repositories/orgfreedomoffice-4711
...
----
  <repositories>
    <repository>
      ...
      <url>https://oss.sonatype.org/content/repositories/orgfreedomoffice-4711</url>
    </repository>
  </repositories>
----
...
https://oss.sonatype.org/content/repositories/releases/org/freedomoffice/

** It may take up to 2 hours until the artifacts appear on Maven
Central:

http://central.maven.org/maven2/org/freedomoffice/
...
** Select `org.freedomoffice` as `Project`.
