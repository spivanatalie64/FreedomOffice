...
cp -r "${PREFIXDIR?}"/lib/freedomoffice /app/
ln -s /app/freedomoffice/program/FREEDOMOFFICE /app/bin/freedomoffice

mkdir -p /app/share/applications
"${SRCDIR?}"/solenv/bin/assemble-flatpak-desktop.sh "${PREFIXDIR?}"/share/applications/ \
 /app/share/applications/ "${1?}"

## icons/hicolor/*/apps/freedomoffice-* ->
## icons/hicolor/*/apps/$1-*:
mkdir -p /app/share/icons
for i in "${PREFIXDIR?}"/share/icons/hicolor/*/apps/freedomoffice-*
do
 mkdir -p \
 ...
 cp -a "$i" \
  "$(dirname /app/share/icons/hicolor/"${i#"${PREFIXDIR?}"/share/icons/hicolor/}")"/"${1?}"."${i##*/apps/freedomoffice-}"
done

mkdir -p /app/share/runtime/locale
for i in $(ls /app/freedomoffice/program/resource)
do
  lang="${i%[_@]*}"
  mkdir -p /app/share/runtime/locale/"${lang}"/resource
  mv /app/freedomoffice/program/resource/"${i}" /app/share/runtime/locale/"${lang}"/resource
  ln -s ../../../share/runtime/locale/"${lang}"/resource/"${i}" /app/freedomoffice/program/resource
done
...
shopt -s nullglob
for i in /app/freedomoffice/share/registry/Langpack-*.xcd /app/freedomoffice/share/registry/res/{fcfg_langpack,registry}_*.xcd
do
