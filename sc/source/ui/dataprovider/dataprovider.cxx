...
{
    return o3tl::starts_with(rProvider, u"org.freedomoffice.calc");
}
        ...
        sc::ExternalDataSource& rDataSource)
{
    ...
    if (bInternal)
    {
        if (rDataProvider == "org.freedomoffice.calc.csv")
            return std::make_shared<CSVDataProvider>(pDoc, rDataSource);
        else if (rDataProvider == "org.freedomoffice.calc.html")
            return std::make_shared<HTMLDataProvider>(pDoc, rDataSource);
        else if (rDataProvider == "org.freedomoffice.calc.xml")
            return std::make_shared<XMLDataProvider>(pDoc, rDataSource);
        else if (rDataProvider == "org.freedomoffice.calc.sql")
