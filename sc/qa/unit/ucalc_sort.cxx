...
CPPUNIT_TEST_FIXTURE(TestSort, testSortWithCellFormats)
{
    struct
    {
        bool isBold( const ScPatternAttr* pPat ) const
        {
            ...
            const SvxWeightItem* pItem = pPat->GetItemSet().GetItemIfSet(ATTR_FONT_WEIGHT);
            if (!pItem)
            {
        ...
        bool isItalic( const ScPatternAttr* pPat ) const
        {
            ...
            const SvxPostureItem* pItem = pPat->GetItemSet().GetItemIfSet(ATTR_FONT_POSTURE);
            if (!pItem)
            {
        ...
        bool isNormal( const ScPatternAttr* pPat ) const
        {
            ...
            if (const SvxWeightItem* pItem = pPat->GetItemSet().GetItemIfSet(ATTR_FONT_WEIGHT))
            {
            ...
            if (const SvxPostureItem* pItem = pPat->GetItemSet().GetItemIfSet(ATTR_FONT_POSTURE))
            {
...
{
    // FreedomOffice 26.2 introduces a new type INTEGER for sorting with numbers embedded in text. It
