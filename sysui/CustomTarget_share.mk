...
$(share_WORKDIR)/%/LOKDocView-0.1.gir: \
		$(call gb_Library_get_target,freedomofficekitgtk)
	mkdir -p $(dir $@)
	PYTHONWARNINGS=default g-ir-scanner "${SRCDIR}/include/FreedomOfficeKit/FreedomOfficeKitGtk.h" \
				 "${SRCDIR}/freedomofficekit/source/gtk/lokdocview.cxx" \
                 `${PKG_CONFIG} --cflags-only-I gobject-introspection-1.0 gtk+-3.0` \
				 -I"${SRCDIR}/include/" \
                 --include=GLib-2.0 --include=GObject-2.0 --include=Gio-2.0 \
                 --library=freedomofficekitgtk --library-path="${INSTDIR}/program" \
                 ...
                 --c-include="FreedomOfficeKit/FreedomOfficeKitGtk.h" \
