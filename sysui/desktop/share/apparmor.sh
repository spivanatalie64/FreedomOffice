...
# Generally these apparmor profiles target the latest FreedomOffice

INST_ROOT=$1  #Where freedomoffice program folder can be found
...
#Example uses:
#Ubuntu 16.04 with stock FreedomOffice:
# sudo ./sysui/desktop/share/apparmor.sh /usr/lib/freedomoffice/ sysui/desktop/apparmor/ /etc/apparmor.d/ true true

#Ubuntu 16.04, with built debs from FreedomOffice git
# sudo ./sysui/desktop/share/apparmor.sh /opt/freedomofficedev5.2/ sysui/desktop/apparmor/ /etc/apparmor.d/ true true
...
do
    ...
    sed "s!@INSTDIR@!${INST_ROOT%/}!g" "$filename" > "$tourl"
    echo "$tourl"
  if [ "$CHECK" = "true" ]; then
