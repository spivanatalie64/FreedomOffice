...
"""
    ...
    Usage:
        ...
        When Python and FreedomOffice run in the same process (usual case):
            from scriptforge import CreateScriptService

        When Python and FreedomOffice are started in separate processes,
        FreedomOffice being started from console ... (examples for Linux with port = 2026 or pipe = 'MYPIPE')
        Either:
            ./FREEDOMOFFICE --accept='socket,host=localhost,port=2026;urp;'
        or:
            ./FREEDOMOFFICE --accept='pipe,name=MYPIPE;urp;'
        ...
        run it in the usual interactive mode without code change, write:
            from scriptforge import CreateScriptService
        and start FreedomOffice with a dedicated pipe:
            ./FREEDOMOFFICE --accept='pipe,name=PIPE2FREEDOMOFFICE;urp;'
...
class ScriptForge(object, metaclass = _Singleton):
    ...
    # #########################################################################
    library = 'ScriptForge'
    Version = '26.8'  # Version number of the FreedomOffice release containing the actual file
    #
    ...
    defaultpipe = 'PIPE2FREEDOMOFFICE'

    def __init__(self, hostname = '', port = 0, pipe = ''):
        """
            Because singleton, constructor is executed only once while Python active
            Both arguments are mandatory when Python and FreedomOffice run in separate processes
    ...
    def ConnectToLOProcess(cls, hostname = '', port = 0, pipe = ''):
        """
            ...
            the requested FreedomOffice instance
        ...
        if (len(hostname) > 0 and port > 0 and len(pipe) == 0) \
            ...
            except Exception:  # thrown when FreedomOffice specified instance isn't started
                raise SystemExit(
                    "Connection to FreedomOffice failed (%s)" % conn)
            return ctx
            ...
            try:
                ...
                print("Connection to FreedomOffice failed (%s). Assuming interactive mode." % conn)
    ...
    def InvokeSimpleScript(cls, script, *args):
        """
            ...
            Implicit scope: Either
                ...
                "share"                  a module within FreedomOffice Macros  (PYTHON)
            :param script: Either
        ...
        def ParseScript(_script):
            ...
            try:
                ...
                raise RuntimeError(
                    'The script \'{0}\' could not be located in your FreedomOffice installation'.format(_script))
...
class SFScriptForge:
    ...
    class SF_Basic(SFServices, metaclass = _Singleton):
        """
            ...
            Detailed user documentation:
                https://help.freedomoffice.org/latest/en-US/text/sbasic/shared/03/sf_basic.html?DbPAR=BASIC
            """
        # Mandatory class properties for service registration
    ...
    class SF_Exception(SFServices, metaclass = _Singleton):
        ...
        def PythonShell(cls, variables = None, background = 0xFDF6E3, foreground = 0x657B83):
            ...
            if len(ext.getPackageLocation(apso)) > 0:
                ...
                # The APSO extension could not be located in your FreedomOffice installation
    ...
    class SF_Session(SFServices, metaclass = _Singleton):
        ...
        SCRIPTISSHARED = 'share'  # in FreedomOffice macros (Python)
    ...
    class SF_UI(SFServices, metaclass = _Singleton):
        """
            ...
            different windows composing the whole FreedomOffice application:
                - Windows selection
...
class SFDialogs:
    ...
    class SF_Dialog(SFServices):
        """
            ...
            by code (Terminate()) or until the FreedomOffice application stops.
            """
        # Mandatory class properties for service registration
        serviceimplementation = 'basic'
...
def CreateScriptService(service, *args, **kwargs):
    ...
    #       ScriptForge() is optional in user scripts when Python process inside FreedomOffice process
