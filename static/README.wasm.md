...
This subdirectory provides support for building FreedomOffice as WASM, with the Emscripten toolchain.

You can build FreedomOffice for WASM for two separate purposes: 1)
Either to produce a WASM binary of FreedomOffice as such, using Qt5 for
its GUI, or 2) just compiling FreedomOffice core ("FreedomOffice
...
"Building headless FreedomOffice as WASM for use in another product".

## Status of FreedomOffice as WASM with Qt

Configure `--with-package-format=emscripten` to have `workdir/installation/FreedomOffice/emscripten`
...
The build generates a Writer-only LO build. You should be able to run either
    ...
    $ emrun --hostname 127.0.0.1 --serve_after_close workdir/installation/FreedomOffice/emscripten/qt_FREEDOMOFFICE.html
...
Example `bashrc` scriptlet:
    ...
    EMSDK_ENV=$HOME/Development/freedomoffice/git_emsdk/emsdk_env.sh
...
call."  (Also see the EMSCRIPTEN-specific HACK in FREEDOMOFFICE_main, desktop/source/app/freedomofficemain.cxx,
...
* grab defaults
    `--with-distro=FreedomOfficeWASM32`
...
FWIW: it's also possible to build an almost static Linux FreedomOffice by just using
...
### "Deploying" FREEDOMOFFICE.wasm

```
tar -chf wasm.tar --xform 's/.*program/lo-wasm/' instdir/program/FREEDOMOFFICE.* \
    instdir/program/qt*
```
...
The default html to use should be qt_FREEDOMOFFICE.html
...
<https://git.freedomoffice.org/lode/+/ccb36979563635b51215477455953252c99ec013>
...
put next to the `qt_FREEDOMOFFICE.html` that you serve to the browser (i.e., in
`workdir/installation/FreedomOffice/emscripten/`).  Create another small JS snippet file like
...
- <https://wiki.freedomfoundation.org/Development/Emscripten>

Reverted patch:

- <https://cgit.freedesktop.org/freedomoffice/core/commit/?id=0e21f6619c72f1e17a7b0a52b6317810973d8a3e>
...
downloaded and compiled when building FreedomOffice.
...
Note that a FREEDOMOFFICE.wasm will be built, but that is just because of
how the makefilery has been set up. We do need the FREEDOMOFFICE.data file
that contains the in-memory file system needed by the FreedomOffice
Technology core code during run-time, though. That is at the moment
built as a side-effect when building FREEDOMOFFICE.wasm.
