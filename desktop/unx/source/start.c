...
static ChildInfo * child_spawn(Args *args, sal_Bool bAllArgs, sal_Bool bWithStatus)
{
    ...
    rtl_uString_newFromAscii(&pTmp, "FREEDOMOFFICE.BIN");
...
static int child_get_exit_code(ChildInfo *info)
{
    ...
    {
        fprintf(stderr, "Warning: failed to fetch freedomoffice exit status\n");
        return -1;
    }
...
static void system_checks(void)
{
    ...
    {
        fprintf(stderr, "ERROR: /proc not mounted - FreedomOffice is unlikely to work well if at all\n");
        exit(1);
    }
#endif
}
...
static void sigterm_handler(int ignored)
{
    ...
    if (g_pProcess) {
        ...
        // forward SIGTERM to FREEDOMOFFICE.BIN and give it a chance to semi-gracefully exit
        ...
        if (!SigTermSucceded) {
            osl_terminateProcess(g_pProcess); // uses SIGKILL to terminate FREEDOMOFFICE.BIN
