...
#include <FreedomOfficeKit/FreedomOfficeKitEnums.h>
...
// We also need to hackily be able to start the main freedomoffice thread:
#include "../app/freedomofficemain.h"
...
static LibFreedomOffice_Impl *gImpl = nullptr;
static bool lok_preinit_2_called = false;
static bool gUseCompactFonts = false;
static std::weak_ptr< FreedomOfficeKitClass > gOfficeClass;
static std::weak_ptr< FreedomOfficeKitDocumentClass > gDocumentClass;
...
RectangleAndPart RectangleAndPart::Create(const OString& rPayload)
{
    ...
    if (rPayload.startsWith("EMPTY")) // payload starts with "EMPTY"
    {
        ...
        if (comphelper::FreedomOfficeKit::isPartInInvalidation())
        {
...
static void doc_destroy(FreedomOfficeKitDocument* pThis);
static int doc_saveAs(FreedomOfficeKitDocument* pThis, const char* pUrl, const char* pFormat, const char* pFilterOptions);
static int doc_getDocumentType(FreedomOfficeKitDocument* pThis);
static int doc_getParts(FreedomOfficeKitDocument* pThis);
static char* doc_getPartPageRectangles(FreedomOfficeKitDocument* pThis);
static int doc_getPart(FreedomOfficeKitDocument* pThis);
static void doc_setPart(FreedomOfficeKitDocument* pThis, int nPart);
static void doc_selectPart(FreedomOfficeKitDocument* pThis, int nPart, int nSelect);
static void doc_moveSelectedParts(FreedomOfficeKitDocument* pThis, int nPosition, bool bDuplicate);
static char* doc_getPartName(FreedomOfficeKitDocument* pThis, int nPart);
static void doc_setPartMode(FreedomOfficeKitDocument* pThis, int nPartMode);
static int doc_getEditMode(FreedomOfficeKitDocument* pThis);
static void doc_paintTile(FreedomOfficeKitDocument* pThis,
                          unsigned char* pBuffer,
                          const int nCanvasWidth, const int nCanvasHeight,
                          const int nTilePosX, const int nTilePosY,
                          const int nTileWidth, const int nTileHeight);
static void doc_paintPartTile(FreedomOfficeKitDocument* pThis,
                              unsigned char* pBuffer,
                              const int nPart,
                              const int nMode,
                              const int nCanvasWidth, const int nCanvasHeight,
                              const int nTilePosX, const int nTilePosY,
                              const int nTileWidth, const int nTileHeight);
static int doc_getTileMode(FreedomOfficeKitDocument* pThis);
static void doc_getDocumentSize(FreedomOfficeKitDocument* pThis,
                                long* pWidth,
                                long* pHeight);
static void doc_getDataArea(FreedomOfficeKitDocument* pThis,
                            long nTab,
                            long* pCol,
                            long* pRow);
static void doc_initializeForRendering(FreedomOfficeKitDocument* pThis,
                                       const char* pArguments);

static void doc_registerCallback(FreedomOfficeKitDocument* pThis,
                                FreedomOfficeKitCallback pCallback,
                                void* pData);
static void doc_postKeyEvent(FreedomOfficeKitDocument* pThis,
                             int nType,
                             int nCharCode,
                             int nKeyCode);
static void doc_setBlockedCommandList(FreedomOfficeKitDocument* pThis,
                                int nViewId,
                                const char* blockedCommandList);

static void doc_postWindowExtTextInputEvent(FreedomOfficeKitDocument* pThis,
                                            unsigned nWindowId,
                                            int nType,
                                            const char* pText);
static void doc_removeTextContext(FreedomOfficeKitDocument* pThis,
                                  unsigned nLOKWindowId,
                                  int nCharBefore,
                                  int nCharAfter);
static void doc_postWindowKeyEvent(FreedomOfficeKitDocument* pThis,
                                   unsigned nLOKWindowId,
                                   int nType,
                                   int nCharCode,
                                   int nKeyCode);
static void doc_postMouseEvent (FreedomOfficeKitDocument* pThis,
                                int nType,
                                int nX,
                                int nY,
                                int nCount,
                                int nButtons,
                                int nModifier);
static void doc_postWindowMouseEvent (FreedomOfficeKitDocument* pThis,
                                      unsigned nLOKWindowId,
                                      int nType,
                                      int nX,
                                      int nY,
                                      ...
                                      int nButtons,
                                      int nModifier);
static void doc_postWindowGestureEvent(FreedomOfficeKitDocument* pThis,
                                      unsigned nLOKWindowId,
                                      const char* pType,
                                      int nX,
                                      int nY,
                                      int nOffset);
static void doc_postUnoCommand(FreedomOfficeKitDocument* pThis,
                               const char* pCommand,
                               const char* pArguments,
                               bool bNotifyWhenFinished);
static void doc_setWindowTextSelection(FreedomOfficeKitDocument* pThis,
                                       unsigned nLOKWindowId,
                                       bool swap,
                                       int nX,
                                       int nY);
static void doc_setTextSelection (FreedomOfficeKitDocument* pThis,
                                  int nType,
                                  int nX,
                                  int nY);
static char* doc_getTextSelection(FreedomOfficeKitDocument* pThis,
                                  const char* pMimeType,
                                  char** pUsedMimeType);
static int doc_getSelectionType(FreedomOfficeKitDocument* pThis);
static int doc_getSelectionTypeAndText(FreedomOfficeKitDocument* pThis,
                                       const char* pMimeType,
                                       char** pText,
                                       char** pUsedMimeType);
static int doc_getClipboard (FreedomOfficeKitDocument* pThis,
                             const char **pMimeTypes,
                             size_t      *pOutCount,
                             char      ***pOutMimeTypes,
                             size_t     **pOutSizes,
                             char      ***pOutStreams);
static int doc_setClipboard (FreedomOfficeKitDocument* pThis,
                             const size_t   nInCount,
                             const char   **pInMimeTypes,
                             const size_t  *pInSizes,
                             const char   **pInStreams);
static bool doc_paste(FreedomOfficeKitDocument* pThis,
                      const char* pMimeType,
                      const char* pData,
                      size_t nSize);
static void doc_setGraphicSelection (FreedomOfficeKitDocument* pThis,
                                  int nType,
                                  int nX,
                                  int nY);
static void doc_resetSelection (FreedomOfficeKitDocument* pThis);
static char* doc_getCommandValues(FreedomOfficeKitDocument* pThis, const char* pCommand);
static void doc_setClientZoom(FreedomOfficeKitDocument* pThis,
                                    int nTilePixelWidth,
                                    int nTilePixelHeight,
                                    int nTileTwipWidth,
                                    int nTileTwipHeight);
static void doc_setClientVisibleArea(FreedomOfficeKitDocument* pThis, int nX, int nY, int nWidth, int nHeight);
static void doc_setOutlineState(FreedomOfficeKitDocument* pThis, bool bColumn, int nLevel, int nIndex, bool bHidden);
static int doc_createView(FreedomOfficeKitDocument* pThis);
static int doc_createViewWithOptions(FreedomOfficeKitDocument* pThis, const char* pOptions);
static void doc_destroyView(FreedomOfficeKitDocument* pThis, int nId);
static void doc_setView(FreedomOfficeKitDocument* pThis, int nId);
static int doc_getView(FreedomOfficeKitDocument* pThis);
static int doc_getViewsCount(FreedomOfficeKitDocument* pThis);
static bool doc_getViewIds(FreedomOfficeKitDocument* pThis, int* pArray, size_t nSize);
static void doc_setViewLanguage(FreedomOfficeKitDocument* pThis, int nId, const char* language);
static unsigned char* doc_renderFontOrientation(FreedomOfficeKitDocument* pThis,
                          const char *pFontName,
                          const char *pChar,
                          int* pFontWidth,
                          int* pFontHeight,
                          int pOrientation);
static unsigned char* doc_renderFont(FreedomOfficeKitDocument* pThis,
                          const char *pFontName,
                          const char *pChar,
                          int* pFontWidth,
                          int* pFontHeight);
static char* doc_getPartHash(FreedomOfficeKitDocument* pThis, int nPart);

static void doc_paintWindow(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId, unsigned char* pBuffer,
                            const int nX, const int nY,
                            const int nWidth, const int nHeight);

static void doc_paintWindowDPI(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId, unsigned char* pBuffer,
                               const int nX, const int nY,
                               const int nWidth, const int nHeight,
                               const double fDPIScale);

static void doc_paintWindowForView(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId, unsigned char* pBuffer,
                                   const int nX, const int nY,
                                   const int nWidth, const int nHeight,
                                   const double fDPIScale, int viewId);

static void doc_postWindow(FreedomOfficeKitDocument* pThis, unsigned
 nLOKWindowId, int nAction, const char* pData);

static char* doc_getPartInfo(FreedomOfficeKitDocument* pThis, int nPart);

static bool doc_insertCertificate(FreedomOfficeKitDocument* pThis,
                                  const unsigned char* pCertificateBinary,
                                  const int nCertificateBinarySize,
                                  const unsigned char* pPrivateKeyBinary,
                                  const int nPrivateKeyBinarySize);

static bool doc_addCertificate(FreedomOfficeKitDocument* pThis,
                                 const unsigned char* pCertificateBinary,
                                 const int nCertificateBinarySize);

static int doc_getSignatureState(FreedomOfficeKitDocument* pThis);

static size_t doc_renderShapeSelection(FreedomOfficeKitDocument* pThis, char** pOutput);

static void doc_resizeWindow(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId,
                             const int nWidth, const int nHeight);

static void doc_completeFunction(FreedomOfficeKitDocument* pThis, const char*);


static void doc_sendFormFieldEvent(FreedomOfficeKitDocument* pThis,
                                   const char* pArguments);

static bool doc_renderSearchResult(FreedomOfficeKitDocument* pThis,
                                 const char* pSearchResult, unsigned char** pBitmapBuffer,
                                 int* pWidth, int* pHeight, size_t* pByteSize);

static void doc_sendContentControlEvent(FreedomOfficeKitDocument* pThis, const char* pArguments);

static void doc_setViewTimezone(FreedomOfficeKitDocument* pThis, int nId, const char* timezone);

static void doc_setViewReadOnly(FreedomOfficeKitDocument* pThis, int nId, const bool readonly);

static void doc_setAccessibilityState(FreedomOfficeKitDocument* pThis, int nId, bool bEnabled);

static char* doc_getA11yFocusedParagraph(FreedomOfficeKitDocument* pThis);

static int doc_getA11yCaretPosition(FreedomOfficeKitDocument* pThis);

static char* doc_getPresentationInfo(FreedomOfficeKitDocument* pThis);
...
static void doc_postSlideshowCleanup(FreedomOfficeKitDocument* pThis);

static bool doc_renderNextSlideLayer(
    FreedomOfficeKitDocument* pThis, unsigned char* pBuffer, bool* bIsBitmapLayer, double* pScale, char** pJsonMsg);

static void doc_setViewOption(FreedomOfficeKitDocument* pDoc, const char* pOption, const char* pValue);

static void doc_setColorPreviewState(FreedomOfficeKitDocument* pThis, int nId, bool bEnabled);

} // extern "C"

namespace {
ITiledRenderable* getTiledRenderable(FreedomOfficeKitDocument* pThis)
{
 ...
 */
rtl::Reference<LOKClipboard> forceSetClipboardForCurrentView(FreedomOfficeKitDocument *pThis)
{
    ...
    , mnDocumentId(nDocumentId)
{
    ...
    if (!m_pDocumentClass)
    {
        m_pDocumentClass = std::make_shared<FreedomOfficeKitDocumentClass>();

        m_pDocumentClass->nSize = sizeof(FreedomOfficeKitDocumentClass);
...
{
    if (comphelper::FreedomOfficeKit::isForkedChild())
    {
...
CallbackFlushHandler::CallbackFlushHandler(FreedomOfficeKitDocument* pDocument, FreedomOfficeKitCallback pCallback, void* pData)
...
void CallbackFlushHandler::resetUpdatedTypePerViewId( int nType, int nViewId )
{
    ...
    if( nType == LOK_CALLBACK_INVALIDATE_VISIBLE_CURSOR && !comphelper::FreedomOfficeKit::isViewIdForVisCursorInvalidation())
        allViewIds = true;
    if( !allViewIds )
    {
...
static void                    lo_destroy       (FreedomOfficeKit* pThis);
static int                     lo_initialize    (FreedomOfficeKit* pThis, const char* pInstallPath, const char* pUserProfilePath);
static FreedomOfficeKitDocument* lo_documentLoad  (FreedomOfficeKit* pThis, const char* pURL);
static char *                  lo_getError      (FreedomOfficeKit* pThis);
static void                    lo_freeError     (char* pFree);
static FreedomOfficeKitDocument* lo_documentLoadWithOptions  (FreedomOfficeKit* pThis,
                                                           const char* pURL,
                                                           const char* pOptions);
static void                    lo_registerCallback (FreedomOfficeKit* pThis,
                                                    FreedomOfficeKitCallback pCallback,
                                                    void* pData);
static char* lo_getFilterTypes(FreedomOfficeKit* pThis);
static void                    lo_setOptionalFeatures(FreedomOfficeKit* pThis, unsigned long long features);
static void                    lo_setDocumentPassword(FreedomOfficeKit* pThis,
                                                       const char* pURL,
                                                       const char* pPassword);
static char*                   lo_getVersionInfo(FreedomOfficeKit* pThis);
static int                     lo_runMacro      (FreedomOfficeKit* pThis, const char* pURL);

static bool lo_signDocument(FreedomOfficeKit* pThis,
                                   const char* pUrl,
                                   const unsigned char* pCertificateBinary,
                                   const int nCertificateBinarySize,
                                   const unsigned char* pPrivateKeyBinary,
...
static char* lo_extractRequest(FreedomOfficeKit* pThis,
                                   const char* pFilePath);

static void lo_trimMemory(FreedomOfficeKit* pThis, int nTarget);

static void*
lo_startURP(FreedomOfficeKit* pThis, void* pReceiveURPFromLOContext, void* pSendURPToLOContext,
...
static void lo_stopURP(FreedomOfficeKit* pThis, void* pSendURPToLOContext);

static int lo_joinThreads(FreedomOfficeKit* pThis);

static void lo_startThreads(FreedomOfficeKit* pThis);

static void lo_setForkedChild(FreedomOfficeKit* pThis, bool bIsChild);

static void lo_runLoop(FreedomOfficeKit* pThis,
                       FreedomOfficeKitPollCallback pPollCallback,
                       FreedomOfficeKitWakeCallback pWakeCallback,
                       void* pData);

static void lo_registerAnyInputCallback(FreedomOfficeKit* pThis,
                       FreedomOfficeKitAnyInputCallback pAnyInputCallback,
                       void* pData);

static void lo_registerFileSaveDialogCallback(FreedomOfficeKit* pThis,
                       FreedomOfficeKitFileSaveDialogCallback pFileSaveDialogCallback);

static void lo_setOption(FreedomOfficeKit* pThis, const char* pOption, const char* pValue);

static void lo_dumpState(FreedomOfficeKit* pThis, const char* pOptions, char** pState);

static char* lo_extractDocumentStructureRequest(FreedomOfficeKit* pThis, const char* pFilePath,
                                                const char* pFilter);

static int lo_getDocsCount(FreedomOfficeKit* pThis);

LibFreedomOffice_Impl::LibFreedomOffice_Impl()
...
{
    if(!m_pOfficeClass) {
        m_pOfficeClass = std::make_shared<FreedomOfficeKitClass>();
        m_pOfficeClass->nSize = sizeof(FreedomOfficeKitClass);
...
static FreedomOfficeKitDocument* lo_documentLoadWithOptions(FreedomOfficeKit* pThis, const char* pURL, const char* pOptions)
{
    ...
    LibFreedomOffice_Impl* pLib = static_cast<LibFreedomOffice_Impl*>(pThis);
    ...
    try
    {
        ...
        if (!aLanguage.isEmpty() && LanguageTag::isValidBcp47(aLanguage, nullptr))
        {
            ...
            comphelper::FreedomOfficeKit::setLanguageTag(LanguageTag(aLanguage));
            comphelper::FreedomOfficeKit::setLocale(LanguageTag(aLanguage));
        ...
        if (!aClientVisibleArea.isEmpty())
        {
            ...
            if (aTokens.size() >= 4)
            {
                ...
                comphelper::FreedomOfficeKit::setInitialClientVisibleArea(aRectangle);
            }
        }
        ...
        */
        ...
        comphelper::FreedomOfficeKit::setDocId(ViewShellDocId(nThisDocumentId));
        ...
        assert(comphelper::FreedomOfficeKit::getDocId() == ViewShellDocId(nThisDocumentId) && "incorrect docid set on document");
...
static char* lo_extractRequest(FreedomOfficeKit* /*pThis*/, const char* pFilePath)
{
    uno::Reference<frame::XDesktop2> xComponentLoader = frame::Desktop::create(xContext);
...
static void*
lo_startURP(FreedomOfficeKit* /* pThis */, void* pRecieveFromLOContext, void* pSendToLOContext,
            int (*fnReceiveURPFromLO)(void* pContext, const signed char* pBuffer, int nLen),
            int (*fnSendURPToLO)(void* pContext, signed char* pBuffer, int nLen))
{
...
static void lo_stopURP(FreedomOfficeKit* /* pThis */,
...
static int joinThreads(JoinThreads eCategory)
{
    ...
    auto joinable = dynamic_cast<comphelper::FreedomOfficeKit::ThreadJoinable *>(xLangSrv.get());
    ...
    if (eCategory == JoinThreads::ALL)
    {
        ...
        joinable = dynamic_cast<comphelper::FreedomOfficeKit::ThreadJoinable *>(ucpWebdav.get());
        if (joinable && !joinable->joinThreads())
            return 0;
        ...
        joinable = dynamic_cast<comphelper::FreedomOfficeKit::ThreadJoinable *>(progressThread.get());
        if (joinable && !joinable->joinThreads())
            return 0;
    }
...
static void lo_startThreads(FreedomOfficeKit* /* pThis */)
{
    ...
    auto ucpWebdav = xContext->getServiceManager()->createInstanceWithContext(
        "com.sun.star.ucb.WebDAVManager", xContext);
    auto joinable = dynamic_cast<comphelper::FreedomOfficeKit::ThreadJoinable *>(ucpWebdav.get());
    if (joinable)
...
static void lo_setForkedChild(FreedomOfficeKit* /* pThis */, bool bIsChild)
{
    comphelper::FreedomOfficeKit::setForkedChild(bIsChild);
    if (bIsChild)
...
static SfxObjectShell* getSfxObjectShell(FreedomOfficeKitDocument* pThis)
{
    LibLODocument_Impl* pDocument = static_cast<LibLODocument_Impl*>(pThis);
    if (!pDocument)
        return nullptr;
...
static int doc_saveAs(FreedomOfficeKitDocument* pThis, const char* sUrl, const char* pFormat, const char* pFilterOptions)
{
...
static void doc_setPartImpl(FreedomOfficeKitDocument* pThis, int nPart, bool bAllowChangeFocus = true)
{
...
static void doc_setPartMode(FreedomOfficeKitDocument* pThis,
                            int nPartMode)
{
                          ...
                          const int nTileWidth, const int nTileHeight)
{
    ...
    // pixels.
    comphelper::ScopeGuard dpiScaleGuard([]() { comphelper::FreedomOfficeKit::setDPIScale(1.0); });
...
inline static ITiledRenderable* getDocumentPointer(FreedomOfficeKitDocument* pThis)
{
    ITiledRenderable* pDoc = getTiledRenderable(pThis);
...
inline static int getFirstViewIdAsFallback(FreedomOfficeKitDocument* pThis)
{
...
inline static int getAlternativeViewForPaint(FreedomOfficeKitDocument* pThis, ITiledRenderable* pDoc, const SfxViewShell* pCurrentViewShell,
...
static int doc_getTileMode(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/)
{
    SetLastExceptionMsg();
                                 ...
                                 void* pData)
{
    ...
    if (pCallback != nullptr)
    {
        ...
        pViewShell->setFreedomOfficeKitViewCallback(pDocument->mpCallbackFlushHandlers[nView].get());
...
static char* getPostItsPos(FreedomOfficeKitDocument* pThis)
{
    SetLastExceptionMsg();
    ITiledRenderable* pDoc = getTiledRenderable(pThis);
    if (!pDoc)
...
static char* getRulerState(FreedomOfficeKitDocument* pThis)
{
    SetLastExceptionMsg();
    ITiledRenderable* pDoc = getTiledRenderable(pThis);
    if (!pDoc)
...
static char* getAllPartSize(FreedomOfficeKitDocument* pThis)
{
    SetLastExceptionMsg();
    ITiledRenderable* pDoc = getTiledRenderable(pThis);
    if (!pDoc)
...
static void doc_postKeyEvent(FreedomOfficeKitDocument* pThis, int nType, int nCharCode, int nKeyCode)
{
...
static void doc_setBlockedCommandList(FreedomOfficeKitDocument* /*pThis*/, int nViewId, const char* blockedCommandList)
{
    SolarMutexGuard aGuard;
    SfxLokHelper::setBlockedCommandList(nViewId, blockedCommandList);
}

static void doc_postWindowExtTextInputEvent(FreedomOfficeKitDocument* pThis, unsigned nWindowId, int nType, const char* pText)
{
...
static void doc_removeTextContext(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId, int nCharBefore, int nCharAfter)
{
    SolarMutexGuard aGuard;
...
static void doc_postWindowKeyEvent(FreedomOfficeKitDocument* /*pThis*/, unsigned nLOKWindowId, int nType, int nCharCode, int nKeyCode)
{
...
/** Class to react on finishing of a dispatched command.
    ...
    @see FreedomOfficeKitCallbackType::LOK_CALLBACK_UNO_COMMAND_RESULT.
*/
...
void LibFreedomOffice_Impl::dumpState(rtl::OStringBuffer &rState)
{
    rState.append("FreedomOfficeKit state:"
                  "\n\tLastExceptionMsg:\t");
...
static void doc_postUnoCommand(FreedomOfficeKitDocument* pThis, const char* pCommand, const char* pArguments, bool bNotifyWhenFinished)
{
...
static void doc_postMouseEvent(FreedomOfficeKitDocument* pThis, int nType, int nX, int nY, int nCount, int nButtons, int nModifier)
{
...
static void doc_postWindowMouseEvent(FreedomOfficeKitDocument* /*pThis*/, unsigned nLOKWindowId, int nType, int nX, int nY, int nCount, int nButtons, int nModifier)
{
...
static void doc_postWindowGestureEvent(FreedomOfficeKitDocument* /*pThis*/, unsigned nLOKWindowId, const char* pType, int nX, int nY, int nOffset)
{
...
static void doc_setTextSelection(FreedomOfficeKitDocument* pThis, int nType, int nX, int nY)
{
...
static void doc_setWindowTextSelection(FreedomOfficeKitDocument* /*pThis*/, unsigned nLOKWindowId, bool swap, int nX, int nY)
{
...
static bool doc_renderNextSlideLayer(
    FreedomOfficeKitDocument* pThis, unsigned char* pBuffer, bool* pIsBitmapLayer, double* pScale, char** pJsonMessage)
{
    SolarMutexGuard aGuard;
    SetLastExceptionMsg();
...
static void doc_setViewOption(FreedomOfficeKitDocument* pThis, const char* pOption, const char* pValue)
{
...
static char* doc_getTextSelection(FreedomOfficeKitDocument* pThis, const char* pMimeType, char** pUsedMimeType)
{
...
static int doc_getSelectionTypeAndText(FreedomOfficeKitDocument* pThis, const char* pMimeType, char** pText, char** pUsedMimeType)
{
...
static bool doc_paste(FreedomOfficeKitDocument* pThis, const char* pMimeType, const char* pData, size_t nSize)
{
...
static void doc_setGraphicSelection(FreedomOfficeKitDocument* pThis, int nType, int nX, int nY)
{
...
static char* getDocReadOnly(FreedomOfficeKitDocument* pThis)
{
...
static char* getStyles(FreedomOfficeKitDocument* pThis, const char* pCommand)
{
    LibLODocument_Impl* pDocument = static_cast<LibLODocument_Impl*>(pThis);
...
static char* getUndoOrRedo(FreedomOfficeKitDocument* pThis, UndoOrRedo eCommand)
{
    LibLODocument_Impl* pDocument = static_cast<LibLODocument_Impl*>(pThis);
...
static char* getPrintRanges(FreedomOfficeKitDocument* pThis)
{
    ITiledRenderable* pDoc = getTiledRenderable(pThis);
    if (!pDoc)
    {
...
static char* getUndoOrRedoCount(FreedomOfficeKitDocument* pThis, UndoOrRedo eCommand)
{
    LibLODocument_Impl* pDocument = static_cast<LibLODocument_Impl*>(pThis);
...
static char* getTrackedChanges(FreedomOfficeKitDocument* pThis)
{
    LibLODocument_Impl* pDocument = static_cast<LibLODocument_Impl*>(pThis);
...
static char* getTrackedChangeAuthors(FreedomOfficeKitDocument* pThis)
{
    ITiledRenderable* pDoc = getTiledRenderable(pThis);
    if (!pDoc)
    {
...
static void doc_setClientZoom(FreedomOfficeKitDocument* pThis, int nTilePixelWidth, int nTilePixelHeight,
...
static int doc_createViewWithOptions(FreedomOfficeKitDocument* pThis,
                                     const char* pOptions)
{
...
static void doc_destroyView(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* pThis, int nId)
{
...
static void doc_setView(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/, int nId)
{
...
static int doc_getView(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* pThis)
{
...
static int doc_getViewsCount(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* pThis)
{
...
static bool doc_getViewIds(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* pThis, int* pArray, size_t nSize)
{
...
static void doc_setViewLanguage(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/, int nId, const char* language)
{
...
unsigned char* doc_renderFont(FreedomOfficeKitDocument* pThis,
                              const char* pFontName,
                              const char* pChar,
                              int* pFontWidth,
                              int* pFontHeight)
...
unsigned char* doc_renderFontOrientation(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/,
                              const char* pFontName,
                              const char* pChar,
                              int* pFontWidth,
                              int* pFontHeight,
...
static void doc_paintWindow(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId,
                            unsigned char* pBuffer,
                            const int nX, const int nY,
                            const int nWidth, const int nHeight)
{
...
static void doc_paintWindowDPI(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId,
                               unsigned char* pBuffer,
                               const int nX, const int nY,
                               const int nWidth, const int nHeight,
                               const double fDPIScale)
...
static void doc_paintWindowForView(FreedomOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                   ...
                                   const double fDPIScale, int viewId)
{
    ...
    comphelper::FreedomOfficeKit::setDialogPainting(true);
    ...
    comphelper::ScopeGuard dpiScaleGuard([]() { comphelper::FreedomOfficeKit::setDPIScale(1.0); });
    comphelper::FreedomOfficeKit::setDPIScale(fDPIScale);
    ...
    comphelper::FreedomOfficeKit::setDialogPainting(false);
}

static void doc_postWindow(FreedomOfficeKitDocument* /*pThis*/, unsigned nLOKWindowId, int nAction, const char* pData)
{
...
static void doc_completeFunction(FreedomOfficeKitDocument* pThis, const char* pFunctionName)
{
    SolarMutexGuard aGuard;
    SetLastExceptionMsg();
...
static void doc_sendFormFieldEvent(FreedomOfficeKitDocument* /*pThis*/, const char* /*pArguments*/)
{
    return;
}
...
static void doc_setViewTimezone(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/, int nId,
...
static void doc_setViewReadOnly(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/, int nId, const bool readOnly)
{
...
static void doc_setAccessibilityState(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* pThis, int nId, bool nEnabled)
{
    SolarMutexGuard aGuard;
...
static void doc_setColorPreviewState(SAL_UNUSED_PARAMETER FreedomOfficeKitDocument* /*pThis*/, int nId,
                                     bool bEnabled)
{
    SolarMutexGuard aGuard;
...
static char* lo_getFilterTypes(FreedomOfficeKit* pThis)
{
    ...
    LibFreedomOffice_Impl* pImpl = static_cast<LibFreedomOffice_Impl*>(pThis);
...
static void lo_setOptionalFeatures(FreedomOfficeKit* pThis, unsigned long long const features)
{
    ...
    LibFreedomOffice_Impl *const pLib = static_cast<LibFreedomOffice_Impl*>(pThis);
    pLib->mOptionalFeatures = features;
    if (features & LOK_FEATURE_PART_IN_INVALIDATION_CALLBACK)
        comphelper::FreedomOfficeKit::setPartInInvalidation(true);
    if (features & LOK_FEATURE_NO_TILED_ANNOTATIONS)
        comphelper::FreedomOfficeKit::setTiledAnnotations(false);
    if (features & LOK_FEATURE_RANGE_HEADERS)
        comphelper::FreedomOfficeKit::setRangeHeaders(true);
    if (features & LOK_FEATURE_VIEWID_IN_VISCURSOR_INVALIDATION_CALLBACK)
        comphelper::FreedomOfficeKit::setViewIdForVisCursorInvalidation(true);
}
...
static char* lo_getVersionInfo(SAL_UNUSED_PARAMETER FreedomOfficeKit* /*pThis*/)
{
    SetLastExceptionMsg();
...
#elif defined MACOSX
    rtl::Bootstrap::setIniFilename(aAppProgramURL + "/../Resources/" SAL_CONFIGFILE("FREEDOMOFFICE"));
    xContext = cppu::defaultBootstrap_InitialComponentContext();
#else
    rtl::Bootstrap::setIniFilename(aAppProgramURL + "/" SAL_CONFIGFILE("FREEDOMOFFICE"));
    xContext = cppu::defaultBootstrap_InitialComponentContext();
#endif
...
{
    SolarMutexGuard aGuard;
    comphelper::FreedomOfficeKit::setAnyInputCallback(pAnyInputCallback, pData, []() -> int {
...
{
    SolarMutexGuard aGuard;
    comphelper::FreedomOfficeKit::setFileSaveDialogCallback(pFileSaveDialogCallback);
}
...
static void lo_status_indicator_callback(void *data, comphelper::FreedomOfficeKit::statusIndicatorCallbackType type, int percent, const char* pText)
{
    LibFreedomOffice_Impl* pLib = static_cast<LibFreedomOffice_Impl*>(data);
    ...
    switch (type)
    {
    case comphelper::FreedomOfficeKit::statusIndicatorCallbackType::Start:
        pLib->mpCallback(LOK_CALLBACK_STATUS_INDICATOR_START, pText, pLib->mpCallbackData);
        break;
    case comphelper::FreedomOfficeKit::statusIndicatorCallbackType::SetValue:
        ...
        break;
    case comphelper::FreedomOfficeKit::statusIndicatorCallbackType::Finish:
        pLib->mpCallback(LOK_CALLBACK_STATUS_INDICATOR_FINISH, nullptr, pLib->mpCallbackData);
        break;
    }
}

/// Used by preloadData (FreedomOfficeKit) for providing different shortcuts for different languages.
...
/// Used only by FreedomOfficeKit when used by Online to pre-initialize
...
#pragma GCC diagnostic pop
    ...
    // getting the remote FreedomOffice service manager
...
static int lo_initialize(FreedomOfficeKit* pThis, const char* pAppPath, const char* pUserProfileUrl)
{
    enum {
    ...
    { // cf. string lifetime for preinit
        ...
        for (const auto &it : aOpts)
        {
            ...
            if (it == "compact_fonts")
                ...
                comphelper::FreedomOfficeKit::setCompatFlag(
                    comphelper::FreedomOfficeKit::Compat::scNoGridBackground);
            else if (it == "sc_print_twips_msgs")
                comphelper::FreedomOfficeKit::setCompatFlag(
                    comphelper::FreedomOfficeKit::Compat::scPrintTwipsMsgs);
        }
    }
    ...
    if (pThis == nullptr)
    {
        ...
        {
            SAL_INFO("lok", "Create freedomoffice object");
            gImpl = new LibFreedomOffice_Impl();
        }
    }
    else if (bPreInited)
        eStage = SECOND_INIT;
    ...
    if (eStage != SECOND_INIT)
        comphelper::FreedomOfficeKit::setActive();

    if (eStage != PRE_INIT)
        comphelper::FreedomOfficeKit::setStatusIndicatorCallback(lo_status_indicator_callback, pLib);
...
#ifdef IOS
    // A FreedomOffice-using iOS app should have the ICU data file in the app bundle. Initialize ICU
    // to use that.
    ...
    try
    {
        ...
        if (eStage != PRE_INIT)
        {
            ...
            // init in Desktop::Main (run from freedomoffice_main) is done. We can enable
            ...
            SAL_INFO("lok", "Starting freedomoffice_main");
...
SAL_JNI_EXPORT
FreedomOfficeKit *freedomofficekit_hook_2(const char* install_path, const char* user_profile_url)
{
    ...
    return static_cast<FreedomOfficeKit*>(gImpl);
}

SAL_JNI_EXPORT
FreedomOfficeKit *freedomofficekit_hook(const char* install_path)
{
    return freedomofficekit_hook_2(install_path, nullptr);
}
...
SAL_JNI_EXPORT
int lok_preinit_2(const char* install_path, const char* user_profile_url, FreedomOfficeKit** kit)
{
...
static void lo_destroy(FreedomOfficeKit* pThis)
{
    ...
    comphelper::FreedomOfficeKit::setStatusIndicatorCallback(nullptr, nullptr);
    uno::Reference <frame::XDesktop2> xDesktop = frame::Desktop::create ( ::comphelper::getProcessComponentContext() );
    // FIXME: the terminate() call here is a no-op because it detects
    // that FreedomOfficeKit::isActive() and then returns early!
