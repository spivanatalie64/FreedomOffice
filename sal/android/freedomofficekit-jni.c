...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "FreedomOfficeKit", __VA_ARGS__))
#define LOGE(...) ((void)__android_log_print(ANDROID_LOG_ERROR, "FreedomOfficeKit", __VA_ARGS__))
...
extern void Java_org_freedomoffice_android_Bootstrap_putenv(JNIEnv* env, jobject clazz, jstring string);
extern void Java_org_freedomoffice_android_Bootstrap_redirect_1stdio(JNIEnv* env, jobject clazz, jboolean state);

extern FreedomOfficeKit* freedomofficekit_hook(const char* install_path);
...
void
Java_org_freedomoffice_kit_FreedomOfficeKit_putenv
    (JNIEnv* env, jobject clazz, jstring string)
{
    Java_org_freedomoffice_android_Bootstrap_putenv(env, clazz, string);
}

/// Call the same method from Bootstrap.
__attribute__ ((visibility("default")))
void Java_org_freedomoffice_kit_FreedomOfficeKit_redirectStdio
    (JNIEnv* env, jobject clazz, jboolean state)
{
    Java_org_freedomoffice_android_Bootstrap_redirect_1stdio(env, clazz, state);
}

/// Initialize the FreedomOfficeKit.
__attribute__ ((visibility("default")))
jboolean freedomofficekit_initialize(JNIEnv* env,
     jstring dataDir, jstring cacheDir, jstring apkFile, jobject assetManager)
{
    ...
    if (fd != -1) {
        ...
        //Java_org_freedomoffice_android_Bootstrap_redirect_1stdio(NULL, NULL, JNI_TRUE);
    }
    free(fontsConfPath);
    ...
    LOGI("FreedomOfficeKit: freedomofficekit_initialize finished");
...
/// Initialize the FreedomOfficeKit.
__attribute__ ((visibility("default")))
jboolean Java_org_freedomoffice_kit_FreedomOfficeKit_initializeNative
     ...
     jstring dataDir, jstring cacheDir, jstring apkFile, jobject assetManager)
{
    ...
    freedomofficekit_initialize(env, dataDir, cacheDir, apkFile, assetManager);

    // FreedomOfficeKit expects a path to the program/ directory
    ...
    // Initialize FreedomOfficeKit
    if (!freedomofficekit_hook(full_program_dir))
    {
        LOGE("freedomofficekit_hook returned null");
        return JNI_FALSE;
    }

    LOGI("FreedomOfficeKit successfully initialized");

    return JNI_TRUE;
}

__attribute__ ((visibility("default")))
jobject Java_org_freedomoffice_kit_FreedomOfficeKit_getFreedomOfficeKitHandle
    (JNIEnv* env, jobject clazz)
{
    FreedomOfficeKit* aOffice;

    (void) env;
    (void) clazz;

    aOffice = freedomofficekit_hook(full_program_dir);

    return (*env)->NewDirectByteBuffer(env, (void*) aOffice, sizeof(FreedomOfficeKit));
}
