...
#endif
{
    ...
    //text-format FreedomOffice supports for cnp then claim we do and we
...
void VclGtkClipboard::OwnerPossiblyChanged(GdkClipboard* clipboard)
{
    ...
    OString sTunnel = "application/x-freedomoffice-internal-id-" + getPID();
    GdkAtom *targets;
    gint n_targets;
    if (gtk_clipboard_wait_for_targets(clipboard, &targets, &n_targets))
    {
...
public:
    ...
    {
        // the gen backend can have per-frame clipboards which is (presumably) useful for FreedomOffice Online
...
#if GTK_CHECK_VERSION(4, 0, 0)
    /* FreedomOffice likes to think of separators between menu entries, while gtk likes
...
public:
    ...
    virtual void set_modal(bool bModal) override
    {
        ...
        /* if change the dialog modality while its running, then also change the parent FreedomOffice window
...
private:
    ...
    void grab_broken(const GdkEventGrabBroken *event)
    {
        ...
        else if (!g_object_get_data(G_OBJECT(event->grab_window), "g-lo-InstancePopup")) // another FreedomOffice popover took a grab
        {
...
private:
    ...
    void add_to_map(GtkWidget* pToolItem, GtkMenuButton* pMenuButton)
    {
        ...
        if (pMenuButton)
        {
            ...
            // a normal button and FreedomOffice's original ones are very
...
namespace
{
    ...
    struct GtkInstanceTreeIter : public weld::TreeIter
    {
        ...
        int compare(const TreeIter& rOther) const
        {
            const GtkInstanceTreeIter& rOtherGtkIter = static_cast<const GtkInstanceTreeIter&>(rOther);
            assert(m_pTreeModel == rOtherGtkIter.m_pTreeModel && "Comparing iters from different tree models");

            GtkTreePath* pOwnPath = gtk_tree_model_get_path(m_pTreeModel, const_cast<GtkTreeIter*>(&iter));
            GtkTreePath* pOtherPath = gtk_tree_model_get_path(rOtherGtkIter.m_pTreeModel, const_cast<GtkTreeIter*>(&rOtherGtkIter.iter));

            int nRet = gtk_tree_path_compare(pOwnPath, pOtherPath);

            gtk_tree_path_free(pOwnPath);
            gtk_tree_path_free(pOtherPath);
...
public:
    ...
    virtual bool get_iter_first(weld::TreeIter& rIter) const override
    {
        ...
        rGtkIter.m_pTreeModel = m_pTreeModel;
...
private:
    ...
    void launch_signal_selection_changed()
    {
        ...
        //as the spin button value. So any FreedomOffice callbacks on
        //signal-change would happen before the spinbutton value-change occurs.
        //To avoid this, send the signal-change to FreedomOffice to occur after focus-change
        //has been processed
        if (m_pChangeEvent)
...
public:
        ...
        , m_pChangeEvent(nullptr)
    {
        ...
        */
        ...
        // tdf#160028 FreedomOffice embeds RTL/LTR direction markers in currency strings, which defeats the
                           ...
                           weld::TreeIter* pRet) override
    {
        ...
        if (pRet)
        {
            ...
            pGtkRetIter->m_pTreeModel = m_pTreeModel;
    ...
    virtual void copy_iterator(const weld::TreeIter& rSource, weld::TreeIter& rDest) const override
    {
        ...
        rGtkDest.m_pTreeModel = rGtkSource.m_pTreeModel;
...
private:
    ...
    static void signalItemActivated(GtkIconView*, GtkTreePath* pTreePath, gpointer widget)
    {
        GtkInstanceIconView* pThis = static_cast<GtkInstanceIconView*>(widget);
        ...
        GtkTreeIter aIter;
        gtk_tree_model_get_iter(pThis->m_pTreeModel, &aIter, pTreePath);
        pThis->signal_item_activated(GtkInstanceTreeIter(*pThis, pThis->m_pTreeModel, aIter));
    }
...
private:
    ...
    {
        const bool bHideHelp = comphelper::FreedomOfficeKit::isActive() &&
    ...
    //to make sure that happens. Without this, if some other part of FreedomOffice has
...
public:
    ...
    virtual std::unique_ptr<weld::ComboBox> weld_combo_box(const OUString &id) override
    {
        ...
        /* we replace GtkComboBox because of difficulties with too tall menus
           ...
           5) https://bugs.freedomfoundation.org/show_bug.cgi?id=131120
