...
//!
//! This module provides the high-level safe Rust wrapper around FreedomOffice's type system.
//! It includes safe type creation, automatic memory management, and type compatibility testing.
//!
//! ## Key Components
//! - `Type` - Safe Rust wrapper around FreedomOffice's typelib_TypeDescriptionReference
//!
//! ## Features
...
impl Type {
    ...
    /// by the caller. Useful when calling FreedomOffice C APIs directly.
    ...
    /// The comparison is efficient and uses FreedomOffice's native type comparison.
    ///
    /// # Arguments
...
/// (same type class and name). This uses FreedomOffice's native type comparison
...
///
/// Creates a Type from a primitive type class. This uses FreedomOffice's static
