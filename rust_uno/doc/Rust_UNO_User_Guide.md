...
Welcome to the **Rust UNO Bindings** user guide. This document provides comprehensive instructions for using Rust to interact with FreedomOffice via the UNO (Universal Network Objects) API.
...
The Rust UNO bindings provide a safe, high-performance interface to automate FreedomOffice. Unlike Python or Java bindings which rely heavily on runtime reflection, Rust UNO uses **compile-time generated wrappers** ensuring:
...
To use Rust UNO, you must build it as part of FreedomOffice.

### Requirements

- **Rust**: Stable toolchain (install via `rustup`).
- **FreedomOffice Build Deps**: Standard development environment for FreedomOffice.
- **Disk Space**: Sufficient space for a full FreedomOffice build.
...
Run the following commands to generate bindings and build FreedomOffice:
...
graph TD
    ...
    CppBridge -->|Calls| NativeUNO["Native FreedomOffice C++ API"]
...
3. **Native UNO**: The actual FreedomOffice implementation.
...
FreedomOffice uses `OUString` (UTF-16). The bindings provide efficient conversion:
