...
The Rust UNO project enables FreedomOffice to be scripted and extended using Rust. It works by inspecting the UNO IDL (Interface Definition Language) types and generating:

1. **Rust Wrappers**: Safe, idiomatic Rust structs.
2. **C++ Bridges**: `extern "C"` functions that handle the raw UNO C++ API.

The system is fully integrated into the FreedomOffice build system via `autogen.sh` and `make`.

---

## Codebase Structure

The project is split across the core FreedomOffice codebase:
...
Ensure you have a full FreedomOffice build environment and have run `./autogen.sh --enable-rust-uno`.
...
Testing is integrated into the FreedomOffice startup sequence for basic sanity checks.
