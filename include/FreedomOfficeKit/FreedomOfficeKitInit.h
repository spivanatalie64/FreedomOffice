...
#ifndef INCLUDED_FREEDOMOFFICEKIT_FREEDOMOFFICEKITINIT_H
#define INCLUDED_FREEDOMOFFICEKIT_FREEDOMOFFICEKITINIT_H
...
#include <sys/stat.h>
    ...
    #ifdef __APPLE__
        #define FREEDOMOFFICEAPP_LIB "libfreedomofficeapp.dylib"
        #define MERGED_LIB "libmergedlo.dylib"

        #if (!defined TARGET_OS_IPHONE || TARGET_OS_IPHONE == 0) && (!defined TARGET_OS_OSX || TARGET_OS_OSX == 0)
            #error FreedomOfficeKit is not supported on tvOS, visionOS or watchOS
        #endif
    #else
        #define FREEDOMOFFICEAPP_LIB "libfreedomofficeapp.so"
    ...
    #endif
    #include <windows.h>
    #define FREEDOMOFFICEAPP_LIB "freedomofficeapp.dll"
...
static void *lok_dlopen( const char *install_path, char ** _imp_lib )
{
    ...
    *_imp_lib = NULL;
    ...
    imp_lib_size = partial_length + sizeof(FREEDOMOFFICEAPP_LIB) + sizeof(MERGED_LIB) + 2;
    ...
    strncpy(imp_lib + partial_length, FREEDOMOFFICEAPP_LIB, imp_lib_size - partial_length);

    struct stat st;
    // If FREEDOMOFFICEAPP_LIB exists but is ridiculously small, it is the
...
typedef FreedomOfficeKit *(LokHookFunction)( const char *install_path);

typedef FreedomOfficeKit *(LokHookFunction2)( const char *install_path, const char *user_profile_url );
...
typedef int             (LokHookPreInit2) ( const char *install_path, const char *user_profile_url, FreedomOfficeKit** kit);

#if defined(IOS) || defined(ANDROID) || defined(__EMSCRIPTEN__)
FreedomOfficeKit *freedomofficekit_hook_2(const char* install_path, const char* user_profile_path);
#endif

// install_path is the pathname to the FreedomOffice installation
...
static FreedomOfficeKit *lok_init_2( const char *install_path,  const char *user_profile_url )
{
#if !defined(IOS) && !defined(ANDROID) && !defined(__EMSCRIPTEN__)
    void *dlhandle;
    char *imp_lib;
    ...
    pSym2 = (LokHookFunction2 *) lok_dlsym(dlhandle, "freedomofficekit_hook_2");
    if (!pSym2)
    {
        if (user_profile_url != NULL)
        {
            fprintf( stderr, "the FreedomOffice version in '%s' does not support passing a user profile to the hook function\n",
                     imp_lib );
            lok_dlclose( dlhandle );
            free( imp_lib );
            return NULL;
        }
        pSym = (LokHookFunction *) lok_dlsym( dlhandle, "freedomofficekit_hook" );
        if (!pSym)
        {
...
#else
    return freedomofficekit_hook_2( install_path, user_profile_url );
#endif
}

static LOK_TOLERATE_UNUSED
FreedomOfficeKit *lok_init( const char *install_path )
{
...
#endif // INCLUDED_FREEDOMOFFICEKIT_FREEDOMOFFICEKITINIT_H
