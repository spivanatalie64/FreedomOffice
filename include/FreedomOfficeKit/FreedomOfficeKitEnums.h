...
#ifndef INCLUDED_FREEDOMOFFICEKIT_FREEDOMOFFICEKITENUMS_H
#define INCLUDED_FREEDOMOFFICEKIT_FREEDOMOFFICEKITENUMS_H
...
FreedomOfficeKitSelectionType;

/** Optional features of FreedomOfficeKit, in particular callbacks that block
 *  FreedomOfficeKit until the corresponding reply is received, which would
...
FreedomOfficeKitOptionalFeatures;

// This enumerates the types of callbacks emitted to a FreedomOfficeKit
// object's callback function or to a FreedomOfficeKitDocument object's
...
typedef enum
{
     ...
     * registered for the FreedomOfficeKit (singleton) object, not a
     * FreedomOfficeKitDocument one, because we are in the very progress of
     * loading a document and then constructing a FreedomOfficeKitDocument
     * object.
     */
     ...
     *     "kind": "network" etc.
     *     "code": a structured 32-bit error code, the ErrCode from FreedomOffice's <comphelper/errcode.hxx>
    ...
    /**
     * Informs the FreedomOfficeKit client that a font specified in the
     * document is missing.
     *
    ...
    /**
     * Informs the FreedomOfficeKit client that the background color surrounding
...
FreedomOfficeKitExtTextInputType;

/// Returns the string representation of a FreedomOfficeKitCallbackType enumeration element.
static inline const char* lokCallbackTypeToString(int nType)
{
    switch (static_cast<FreedomOfficeKitCallbackType>(nType))
    {
    ...
    assert(!"Unknown FreedomOfficeKitCallbackType type.");
    return nullptr;
}
...
FreedomOfficeKitMouseEventType;
...
FreedomOfficeKitSetTextSelectionType;
...
FreedomOfficeKitSetGraphicSelectionType;
...
#endif // INCLUDED_FREEDOMOFFICEKIT_FREEDOMOFFICEKITENUMS_H
