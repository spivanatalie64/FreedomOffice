...
#include "FreedomOfficeKit.h"
#include "FreedomOfficeKitEnums.h"
#include "FreedomOfficeKitInit.h"
...
public:
    ...
    FreedomOfficeKitDocument *get() { return mpDoc; }

    /**
     * Get document type.
     *
     * @since FreedomOffice 6.0
     * @return an element of the FreedomOfficeKitDocumentType enum.
     */
    int getDocumentType()
    {
     ...
     *
     * @return an element of the FreedomOfficeKitTileMode enum.
     */
    int getTileMode()
    {
     ...
     */
    void registerCallback(FreedomOfficeKitCallback pCallback, void* pData)
    {
     ...
     *
     * @param nType @see FreedomOfficeKitSetTextSelectionType
     * @param nX horizontal position in document coordinates
     * @param nY vertical position in document coordinates
     */
     ...
     *
     * @return an element of the FreedomOfficeKitSelectionType enum.
     */
    ...
    {
        if (FREEDOMOFFICEKIT_DOCUMENT_HAS(mpDoc, getSelectionTypeAndText))
     ...
     *
     * @param nType @see FreedomOfficeKitSetGraphicSelectionType
     * @param nX horizontal position in document coordinates
     * @param nY vertical position in document coordinates
     */
    ...
    {
        if (FREEDOMOFFICEKIT_DOCUMENT_HAS(mpDoc, createViewWithOptions))
    ...
    unsigned char* renderFont(const char *pFontName,
                          ...
                          int pOrientation=0)
    {
        if (FREEDOMOFFICEKIT_DOCUMENT_HAS(mpDoc, renderFontOrientation))
     ...
     * @param nType see FreedomOfficeKitExtTextInputType
...
/// The lok::Office class represents one started FreedomOfficeKit instance.
class Office
{
private:
    FreedomOfficeKit* mpThis;

public:
    /// A lok::Office is typically created by the lok_cpp_init() function.
    Office(FreedomOfficeKit* pThis) :
        mpThis(pThis)
    {}
     ...
     *        by the documentLoad() itself, and when provided, FreedomOfficeKit
     *        switches the language accordingly first.
     * @since pFilterOptions argument added in FreedomOffice 5.0
     */
    Document* documentLoad(const char* pUrl, const char* pFilterOptions = NULL)
    {
        FreedomOfficeKitDocument* pDoc = NULL;

        if (FREEDOMOFFICEKIT_HAS(mpThis, documentLoadWithOptions))
     ...
     *
     * Use on dynamically allocated data returned by FreedomOfficeKit
     ...
     *
     * @since FreedomOffice 6.0
     * @see FreedomOfficeKitOptionalFeatures
     */
     ...
     * and the @pWakeCallback can be called by internal FreedomOfficeKit threads
     ...
     */
    void runLoop(FreedomOfficeKitPollCallback pPollCallback,
     ...
     * logging functionality" in FreedomOffice internal API
     ...
     *
     * FreedomOfficeKit can get into an unhelpful state at run-time when
     ...
     * Trim memory usage.
     *
     * FreedomOfficeKit caches lots of information from large pixmaps
     ...
     */
    void registerAnyInputCallback(FreedomOfficeKitAnyInputCallback pCallback, void* pData)
    {
        return mpThis->pClass->registerAnyInputCallback(mpThis, pCallback, pData);
    }

    /**
     * Get number of documents of this FreedomOfficeKit.
     */
    int getDocsCount()
    {
     ...
     */
    void registerFileSaveDialogCallback(FreedomOfficeKitFileSaveDialogCallback pCallback)
    {
...
///
/// For information on the parameters, see writeup for lok_init_2 in FreedomOfficeKitInit.h.
inline Office* lok_cpp_init(const char* pInstallPath, const char* pUserProfileUrl = NULL)
{
    FreedomOfficeKit* pThis = lok_init_2(pInstallPath, pUserProfileUrl);
