...
/* checks for the fix from https://gerrit.freedomoffice.org/c/core/+/147660 */
CPPUNIT_TEST_FIXTURE(test::AccessibleTestBase, TestSpecialCharactersDialogFocus)
{
    ...
    auto dialogWaiter = awaitDialog(u"Special Characters", [](Dialog& dialog) {
         ...
         * (https://bugs.freedomfoundation.org/show_bug.cgi?id=153806), we move to another cell
         * so it works. */
...
CPPUNIT_TEST_FIXTURE(test::AccessibleTestBase, BasicTestHyperlinkDialog)
{
    ...
    auto dialogWaiter = awaitDialog(u"Hyperlink", [this](Dialog& dialog) {
        ...
        // Fill in an address
        dialog.postExtTextEventAsync(u"https://freedomoffice.org/"_ustr);
    ...
    CPPUNIT_ASSERT_EQUAL(u"<PARAGRAPH>https://freedomoffice.org/</PARAGRAPH>"_ustr, collectText());
}
