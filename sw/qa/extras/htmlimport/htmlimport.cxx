...
CPPUNIT_TEST_FIXTURE(HtmlImportTest, testInlinedImage)
{
    ...
    for (SwNodeOffset n(0); ; n++)
    {
        ...
        if (SwGrfNode *pGrfNode = pNode->GetGrfNode())
        {
            ...
            // running FreedomOffice but cannot reproduce that in a unit test here. :-(
...
CPPUNIT_TEST_FIXTURE(HtmlImportTest, testMetaIsoDates)
{
    ...
    DateTime aModified(xDocProps->getModificationDate()); // in the legacy format (what FreedomOffice used to write)
...
CPPUNIT_TEST_FIXTURE(HtmlImportTest, testReqIfTable)
{
    ...
    // to see this: FREEDOMOFFICE --infilter="HTML (StarWriter):xhtmlns=reqif-xhtml" sw/qa/extras/htmlimport/data/reqif-table.xhtml
