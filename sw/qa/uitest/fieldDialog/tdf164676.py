...
class Tdf164676(UITestCase):

    def test_tdf164676(self):
        ...
        with self.ui_test.create_doc_in_start_center("writer") as document:
            ...
            with self.ui_test.execute_modeless_dialog_through_command(".uno:InsertField", close_button="cancel") as xDialog:
                ...
                xType.getChild('0').executeAction("SELECT", tuple())
                self.assertEqual("Set variable", get_state_as_dict(xType)['SelectEntryText'])

                xName = xDialog.getChild("name-var")
                xName.executeAction("TYPE", mkPropertyValues({"TEXT": "AA"}))

                xValue = xDialog.getChild("value-var")
                xValue.executeAction("TYPE", mkPropertyValues({"TEXT": "1234"}))
            ...
            # AssertionError: '1234' != ''
            self.assertEqual("1234", document.Text.createEnumeration().nextElement().String)
