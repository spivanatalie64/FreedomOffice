...
void DocxSdrExport::startDMLAnchorInline(const SwFrameFormat* pFrameFormat, const Size& rSize)
{
    ...
    // FreedomOffice behaves different for frames and drawing objects, but MS Office treats frames
    ...
    **/
    ...
    // FixMe: tdf141880. Because FreedomOffice currently cannot handle negative vertical margins, they
    ...
    // ToDo: Exclude cases where FreedomOffice wrap without contour is different
    // from Word XML_wrapSquare or where direct use of distances not possible and workaround
    // will be done using wrapPolygon.
    // ToDo: handle case Writer frame, where contour can be set in FreedomOffice but is not rendered.
    ...
    // concept of a wrap polygon and FreedomOffice has no one internally. So as a workaround, we
